...
const droidHome = process.env.DROID_HOME || `${homedir()}/.droid`;
const stackStatePath = `${process.env.SELF_HOST_STATE_DIR || droidHome}/full-stack-state.json`;
...
if (mode === 'predev') {
  const orchestrated = Boolean(process.env.DROID_STACK_ORCHESTRATOR);
  const explicitlyAllowed = process.env.DROID_ALLOW_ISOLATED_CONSOLE === '1';
  if (!orchestrated && !explicitlyAllowed && stackStateIsActive()) {
    console.error('Refusing bare `npm run dev`: a full local Droid stack appears to be running.');
    console.error('Use `droid-deploy/selfhost/stack.sh repair-console` or `status` instead.');
    console.error(
      'Override only for intentionally isolated Console work: DROID_ALLOW_ISOLATED_CONSOLE=1 npm run dev'
    );
    process.exit(1);
  }
}
...
if (mode === 'prebuild') {
  if (process.env.DROID_ALLOW_LIVE_BUILD === '1') process.exit(0);
  if (sameCheckoutNextDevIsRunning()) {
    ...
    console.error(
      'Override only when no browser/dev server depends on this checkout: DROID_ALLOW_LIVE_BUILD=1 npm run build'
    );
    process.exit(1);
  }
}
