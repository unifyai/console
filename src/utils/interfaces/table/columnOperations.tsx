...
import { sanitizeId, processContext } from '@/lib/logs/columns';

export { sanitizeId, processContext };
...
export function isAllChildrenGrouped(column: Column<any>, grouping: string[]): boolean {
  ...
  const allGrouped =
    children.length > 0 &&
    children.every((child) => grouping.includes(child.columnDef.id as string));
...
export const updateParentVisibility = (
  ...
  if (parentKey) {
    ...
    if (
      (allSiblingsVisible && updatedVisibility[childKey]) ||
      (allSiblingsHidden && !updatedVisibility[childKey])
    ) {
...
export const getImmediateSiblings = (
  ...
  const siblingColumns = allColumns
    .filter(
      (col) =>
        col.id !== column.id &&
        hasSameParent(col.id, column.id) &&
...
export const getImmediateRightNeighbors = (
  ...
  const immediateRightNeighbors = allColumns
    .filter((col) => {
      ...
      return (
        col.id !== column.id &&
        parentID === immediateRightParentID &&
        col.columnDef.meta?.renderedDepth === currentDepth
      );
    })
...
export const getImmediateHiddenRightNeighbors = (
  ...
  return immediateRightNeighbors.filter(
    (neighbor) => neighbor !== column.id && !columnVisibility[neighbor]
  );
};
