...
import { Header, HeaderGroup, Column, Row, Cell, ColumnDef, GroupingState, FilterFnOption, RowSelectionState } from "@tanstack/react-table";
import {
	RowData,
	RowModel,
	createRow,
...
export const findParentHeaderInstance = (
	...
	if (parentGroupIndex >= headerGroups.length) {
		console.warn(`Parent group index ${parentGroupIndex} is out of bounds for header groups (length ${headerGroups.length})`);
		return undefined;
	}
	const parentHeaderGroup = headerGroups[parentGroupIndex]; // Get the correct parent group by index
	if (!parentHeaderGroup) {
		console.warn(`Could not find parent header group at index ${parentGroupIndex} for header ${header.id}`);
		return undefined;
	}
	...
	const parentHeader = parentHeaderGroup.headers.find(
		h => h.column.id === targetParentColumnId
	);
	...
	if (allSubHeadersArePlaceholders) {
	  ...
	  const findMinDepth = (currentHeader: Header<any, unknown>) => {
		...
		if (currentHeader.subHeaders && currentHeader.subHeaders.length > 0) {
		   ...
		   const childrenInNextDepth = currentHeader.subHeaders.filter(sub => sub.depth === currentHeader.depth + 1);
...
export const shouldRenderHeader = (
	...
	const parentColumnHeader = header.column.parent ? table.getHeaderGroups()[groupIndex -1]?.headers.find(h => h.column.id === header.column.parent?.id && !h.isPlaceholder) : undefined;
  
	if (parentColumnHeader) { // Only proceed if a non-placeholder column parent exists in the row above
		...
		if (header.depth > parentColumnHeader.depth && header.depth < parentColumnHeader.depth + parentCalculatedRowSpan) {
