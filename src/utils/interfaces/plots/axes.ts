...
function niceIncrement(min: number, max: number, count = 10) {
  ...
  const bestStep = factors
    .map((f) => base * f)
    .reduce((prev, curr) => (Math.abs(curr - rawStep) < Math.abs(prev - rawStep) ? curr : prev));
  return bestStep;
}
 ...
 */
function formatTimeTypeTick(
  currentTickValue: number,
  previousTickValue: number | undefined,
  dataType: string
) {
  ...
  const [previousTickFormat, currentTickFormat] = [previousTickValue, currentTickValue].map(
    (tick) => formatTimeTypeValue(tick, dataType)
  );
  ...
  switch (dataType) {
    case 'timestamp':
      ...
      [previousParts, currentParts] = [previousTickFormat, currentTickFormat].map((tickFormat) =>
        tickFormat
          .split(' ')
          .flatMap((dateOrTime, i) =>
            i === 0
              ? dateOrTime.split('-')
              : dateOrTime
                  .split(':')
                  .flatMap((timePart, i) => (i === 2 ? timePart.split('.') : timePart))
          )
      );
    ...
    case 'timedelta':
      ...
      [previousParts, currentParts] = [previousTickFormat, currentTickFormat].map((tickFormat) =>
        tickFormat
          .split(', ')
          .flatMap((dayOrTime, i) => (i === 0 ? dayOrTime : dayOrTime.split(':')))
      );
      if (previousParts[0] === currentParts[0]) {
        if (previousParts[1] === currentParts[1]) {
          if (previousParts[2] === currentParts[2]) {
...
export const drawAxes = (
  ...
  if (plotType === 'Bar Chart') {
    ...
    xTickFormatter = d3
      .axisBottom(x as d3.ScaleLinear<number, number, never>)
      .tickValues(xTicks)
      .tickFormat((d, i) => {
        const prevTick = i > 0 ? xTicks[i - 1] : undefined;
        if (xType === 'timestamp' || xType === 'timedelta' || xType === 'time' || xType === 'date')
          return formatTimeTypeTick(d as number, prevTick, xType);
        return reverseX ? formatNumber(-d as number) : formatNumber(d as number);
      }) as any;
  } else {
    xTickFormatter = d3
      .axisBottom(
        x as d3.ScaleLinear<number, number, never> | d3.ScaleLogarithmic<number, number, never>
      )
      .tickValues(xTicks)
      .tickFormat((d, i) => {
  ...
  yTickFormatter = d3
    ...
    .tickValues(
      plotType === 'Histogram' || (plotType === 'Bar Chart' && yTicks.every((tick) => tick >= 0))
        ? yTicks.length > 1
          ? yTicks.slice(1)
          : yTicks
        : (yTicks as number[])
  ...
  xAxis
    ...
    .attr('transform', 'rotate(-20) translate(0, 5)')
  ...
  if (
    ['Line Chart', 'Scatter Plot'].includes(plotType) &&
    Math.min(...xTicks) <= 0 &&
    Math.max(...xTicks) >= 0
  ) {
    zeroXLine
  ...
  if (
    ['Bar Chart', 'Line Chart', 'Scatter Plot'].includes(plotType) &&
    Math.min(...yTicks) <= 0 &&
    Math.max(...yTicks) >= 0
  ) {
    zeroYLine
