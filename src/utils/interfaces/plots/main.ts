  ...
  try {
    ...
    if (plotType === 'Line Chart') {
      if (logs && selectedXAxisProperty && selectedYAxisProperty) {
        ...
        const adjustedScaleX = checkLogScalability(
          logs,
          fields,
          xTable,
        ...
        const adjustedScaleY = checkLogScalability(
          logs,
          fields,
          yTable,
      ...
      if (logs && selectedXAxisProperty && selectedYAxisProperty) {
        // Note: Placeholder text is now handled internally by the scatter plot module
        // based on render mode (SVG/WebGL) and data processing (sampling/culling)
        placeholder.text('');
        const setScaleX = plotTileActions?.setPlotScaleX;
        const setScaleY = plotTileActions?.setPlotScaleY;
        if (!setScaleX || !setScaleY) {
