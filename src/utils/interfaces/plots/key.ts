...
import { GroupingColors } from '@/types/interfaces/plot';
...
export const keyTemplate = (keys: GroupingColors) => {
  const value = (entry: { key: string | null; color: string }) =>
    entry.key?.toString().replace(/^"|"$/g, '') || 'null';
  return keys
    .map(
      (entry) => `
...
export function renderGroupingKey(
  ...
  if (!colors || colors.length === 0) {
    ...
    const setIsMinimized = settingsNode
      ? (settingsNode as any).__setIsGroupingKeyMinimized
      : undefined;
  ...
  const settingsNode = settings.node();
  const isMinimized = settingsNode
    ? ((settingsNode as any).__isGroupingKeyMinimized ?? false)
    : false;
  const setIsMinimized = settingsNode
    ? (settingsNode as any).__setIsGroupingKeyMinimized
    : undefined;
  ...
  // --- Title ---
  header.append('span').attr('class', `text-xs font-semibold mr-2`).text('Grouping Key');

  // --- Button Group ---
  const buttonGroup = header.append('div').attr('class', 'flex items-center gap-1 ml-auto'); // Pushes buttons right
  ...
  if (!isMinimized) {
    const itemsWrapper = container
      ...
      .attr(
        'class',
        'grouping-key-items command-scrollbar mt-1 flex flex-col gap-1 max-h-48 overflow-y-auto'
      );
