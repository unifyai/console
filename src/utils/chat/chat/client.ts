...
export const generateInput = <Args extends Arguments, T extends ModelInputs<Args>>(
  args: Args,
  input: T | null,
  chatHistory: (AssistantMessage | UserMessage)[]
): T => {
  ...
  const chatHistoryArguments = Object.keys(args).filter((key) => args[key].type === 'chat_history');

  chatHistoryArguments.forEach((argKey) => {
    const chatArg = args[argKey] as ArgumentChatHistory;
    const chatInput = chatHistory.map((chatObject) =>
      Object.fromEntries(
        Object.entries(chatArg.structure).map(([key, value]) => {
...
export const generateRequest = <T>(options: MessageOptionsWithoutCallback<T>) => {
  ...
  const body = JSON.stringify({
    model: options.endpoint.router
      ? `router@${['q:1']
          .concat(
            options.endpoint.code
              .split('_')
              .slice(1)
              .map((value, index) => `${params[index]}:${value}`)
          )
          .join('|')}`
      : options.endpoint.code + '@' + options.endpoint.provider,
    ...options.arguments,
    signature: 'chat',
  });
  return {
...
export const getEndpointChat = (endpoint: Endpoint, chatHistory: ChatHistory) => {
  const endpointHistory = chatHistory
    .map((chat) => {
      if ('content' in chat) {
        return chat;
      }
      const checkEndpoint = (message: AssistantMessage) => {
        ...
        if (
          endpoint &&
          'code' in messageEndpoint &&
          'code' in endpoint &&
          'provider' in messageEndpoint &&
          'provider' in endpoint
        ) {
          return (
            messageEndpoint.code == endpoint.code && messageEndpoint.provider == endpoint.provider
          );
        }
  ...
  if (assistantIndex == -1) firstMessageIndex = roles.length - 1;
...
export const getMaxBubbles = (category: string) => {
  if (['2xl', 'xl'].includes(category)) return 3;
  else if (['lg'].includes(category)) return 2;
  return 1;
};
