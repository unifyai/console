...
export async function* race<T>(iterable: AsyncGenerator<T>[]) {
  ...
  const next = (gen: AsyncGenerator<T>) => {
    const promise: any = gen.next().then(({ done, value }) => ({ done, value, gen, promise }));
    return promise;
  };
  const promises: any = generators.reduce((set, gen) => set.add(next(gen)), new Set());
...
const chatWithEndpoint = async function* (
  key: string,
  endpoint: Endpoint,
  ...
  if (response.status != 200) {
    yield {
      ...
      endpoint,
      delta:
        response.statusText == 'Payment Required'
  ...
  while (!done) {
    ...
    if (value) {
      ...
      const chunks = buffer.split(/\s*data:\s*/g).filter((str) => str.length > 0);
      ...
      const stringChunks = (
        jsonChunks.filter((chunk) => typeof chunk !== 'string') as ResponseChunk[]
      ).map(
        (chunk) => chunk?.choices?.[0]?.message?.content || chunk?.choices?.[0]?.delta?.content
      );
  ...
  yield {
    ...
    metrics: lastChunk
      ? {
          ...
          latency: timeToFirstToken,
          throughput:
            timeToCompletion !== undefined && lastChunk.usage?.totalTokens !== undefined
...
const chatWithEndpointWithError = async function* (
  key: string,
  endpoint: Endpoint,
  chatHistory: (AssistantMessage | UserMessage)[],
  parameters: Parameters,
...
export const chat = streamResponse(async function* ({
  key,
  message,
  endpoints,
  parameters,
  prevResponses,
  apiKey,
}: ChatRequest): AsyncGenerator<ChatFrame> {
  if (!key) {
    key = uuid();
  }
  ...
  if (prevResponses.length > 0) history.push(prevResponses);
  history.push({
    content: message,
    role: 'user',
  });
