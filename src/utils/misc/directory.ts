...
export function updateNode (
    ...
    if (currentNode && newNode && tree){
      ...
      flattenedTree = flattenedTree.filter((node: NodeProps) => node.path != currentNode.path && !node.path.startsWith(currentNode.path));
      ...
      const newEntries = flattenedTree
        .filter((node: NodeProps) => 
          node.type === "file" 
          || node.nodes?.length === 0           // Allow empty folders
        )
        .map((f: NodeProps) => {
      ...
      const mapping = newFilesTree.map((file) => ({oldName: oldFilesTree.find(f => f.name === file.name)!.path, newName: file.path }));
