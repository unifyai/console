 ...
 *   2. DROID_ADAPTERS_URL       - per-environment host (staging vs production)
 *
 * There is intentionally no baked-in default: each environment must set
 * DROID_ADAPTERS_URL so the Cloud Run host is never hardcoded in source.
 */
export function getAdaptersBaseUrl(params?: { localAdaptersUrl?: string | null }): string {
  ...
  const configuredAdaptersUrl = cleanUrl(process.env.DROID_ADAPTERS_URL);
  if (configuredAdaptersUrl) {
    return configuredAdaptersUrl;
  }

  throw new Error(
    'DROID_ADAPTERS_URL is not set. Configure it per environment ' +
