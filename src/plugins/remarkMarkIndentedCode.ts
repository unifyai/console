  ...
  return function transform(tree, file) {
    ...
    visit(tree, 'code', (node) => {
      ...
      if (!node.lang && !node.meta) {
        ...
        const hasIndentedPosition =
          node.position && node.position.start.column > 1 && !node.position.indent; // No explicit indent means it might be an implicit indent

        // 3. Check for common patterns of indented code blocks
        const hasIndentedPatterns =
          node.value &&
