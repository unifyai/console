...
export function useTabStreamingQuery(
  ...
  const tabsEtagMap =
    (useTabStreamingQuery as any)._tabsEtag ||
    ((useTabStreamingQuery as any)._tabsEtag = new Map<string, string>());
    ...
    queryFn: async ({ signal }) => {
      try {
        ...
        const res = await fetch(
          `/api/tab?interfaceId=${encodeURIComponent(interfaceId)}&checkpoint=false`,
          {
            method: 'GET',
  ...
  const [stableActiveTabName, setStableActiveTabName] = useState<string | null>(
    activeTabName ?? null
  );
  useEffect(() => {
    const t = setTimeout(() => {
      setStableActiveTabName(activeTabName ?? null);
      debugLog('[useTabStreamingQuery] stableActiveTabName updated', {
        activeTabName,
        stableActiveTabName: activeTabName ?? null,
      });
    }, 250);
  ...
  const enabledActive = !!(
    ...
    activeTabName &&
    projectId &&
    stableActiveTabName === activeTabName
  );
  debugLog('[useTabStreamingQuery] Active query enabled check', {
  ...
  useEffect(() => {
    ...
    try {
      ...
      debugLog('[useTabStreamingQuery] Hydrated tiles into store for tab', {
        tabId,
        count: tiles.length,
      });
    } catch (e) {
  ...
  useEffect(() => {
    ...
    if (needsRefetch) {
      debugLog('[useTabStreamingQuery] Forcing refetch on activation due to prior error/empty', {
        activeTabName,
      });
  ...
  useEffect(() => {
    ...
    debugLog('[useTabStreamingQuery] Scheduling auto-retry for active tab', {
      activeTabName,
      attempts: attempts + 1,
      delay,
    });
  ...
  const prefetchQueue = useMemo(() => {
    ...
    const unprefetchedTabs = prioritizedNonActiveTabs.filter(
      (tab) =>
        !prefetchedTabs.has(tab.name!) &&
        !currentlyPrefetching.has(tab.name!) &&
        !failedPrefetchTabs.has(tab.name!)
    );
    const effectiveConcurrency = Math.max(
      0,
      Math.min(concurrency ?? MAX_CONCURRENT_PREFETCH, prefetchConcurrency)
    );
    ...
    prefetchReady,
    enableNonActivePrefetch,
    prioritizedNonActiveTabs,
    prefetchedTabs,
  ...
  useEffect(() => {
    ...
    if (
      newPrefetching.size !== currentlyPrefetching.size ||
  ...
  useEffect(() => {
    ...
    const networkMaxConcurrency =
      saveData || /2g|3g/.test(effectiveType || '') ? 1 : (concurrency ?? MAX_CONCURRENT_PREFETCH);
  ...
  useEffect(() => {
    ...
    if (
      stillPrefetching.size !== currentlyPrefetching.size ||
    ...
    if (
      newFailed.size !== failedPrefetchTabs.size ||
  ...
  const switchTab = useCallback(
    (tabName: string) => {
      ...
      if (isInstant) {
        ...
        debugLog(
          `[switchTab] Tab ${tabName} is cached (mode=${cachedData?.mode}) - instant switch available`
        );
        return true;
      } else {
        // Need to fetch data, will show loading state
        debugLog(
          `[switchTab] Tab ${tabName} not cached as full (mode=${cachedData?.mode}) - will need to load`
        );
        return false;
      }
    },
  ...
  const refreshTabData = useCallback(
    async (
      ...
      return (
        queryClient.getQueryData<CompleteTabData>([
          'tabCompleteData',
          interfaceId,
          tabName,
          projectId,
