...
export function useSaveTabWithTilesQuery(
  ...
  return useMutation({
    mutationFn: async ({ 
      ...
      try {
        if (tabId) {
          tabCheckpointResult = await tabActions.checkpointById(tabId, description || 'Manual save');
        } else if (interfaceId && tabName) {
          tabCheckpointResult = await tabActions.checkpointByName(interfaceId, tabName, description || 'Manual save');
        }
      } catch (error) {
