...
export function useTabDataOptimistic() {
   ...
   */
  const buildCompleteTabData = useCallback(
    async (
      interfaceId: string,
      tabId: string,
      tabName: string,
      ...
      try {
        ...
        debugLog('[buildCompleteTabData] Tiles cache check:', {
          ...
          isError:
            tilesData && typeof tilesData === 'object' && Object.keys(tilesData).includes('error'),
          listTiles,
        });

        if (
          !tilesData ||
          (typeof tilesData === 'object' && Object.keys(tilesData).includes('error'))
        ) {
          if (listTiles === false) {
            debugLog(
              '[buildCompleteTabData] Skipping tile fetch (listTiles=false), using empty array'
            );
            ...
            try {
              const res = await fetch(
                `/api/tile?tabId=${encodeURIComponent(finalTabId)}&checkpoint=false`,
                {
                  method: 'GET',
            ...
            if (updateCache) {
              debugLog('[buildCompleteTabData] Caching tiles:', {
                tabName,
                tileCount: tilesData.length,
              });
          ...
          try {
            switch (tileData.type) {
              case 'Table': {
                ...
                console.log(
                  `[processTile] ${tileData.name}: tableTileIndex=${tableTileIndex}, fieldsArray.length=${fieldsArray.length}, fields=${JSON.stringify(fields ? Object.keys(fields).slice(0, 3) : null)}...`
                );
        ...
        console.error(
          `[buildCompleteTabData] Error building complete tab data for ${tabName} (ID: ${tabId}):`,
          error
        );
        throw error;
      }
