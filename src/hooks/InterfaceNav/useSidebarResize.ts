...
export function useSidebarResize({
  ...
  const handleMouseDown = React.useCallback(
    (e: React.MouseEvent) => {
      ...
      // Store initial state
      const currentWidthPx = isCompletelyHidden
        ? 0
        : isCollapsed
          ? COLLAPSED_WIDTH_PX
          : toPx(currentWidth);
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isDragging.current) {
        ...
        if (!toggleCooldown.current) {
          ...
          const currentDragWidth = calculateWidth(e, startX.current, startWidth.current);
          ...
          if (isCompletelyHidden && currentDragDirection === 'expand') {
            if (dragDistanceFromToggle.current > 20 && onCompletelyHide) {
              // Small threshold to show
          ...
          if (isCollapsed && !isCompletelyHidden && currentDragDirection === 'expand') {
            // Calculate the drag distance needed to expand
            const expandDistance = isCollapsed
              ? COLLAPSED_WIDTH_PX * 0.5
            ...
            if (dragDistanceFromToggle.current > expandDistance) {
              ...
              const initialWidth = calculateWidth(e, startX.current, startWidth.current);
        ...
        if (isCollapsed && currentDragDirection === 'expand') {
          ...
          if (newWidthPx >= minWidthPx) {
            ...
            const clampedWidthPx = Math.max(minWidthPx, Math.min(maxAllowedWidth, newWidthPx));
        ...
        if (!isCollapsed) {
          // Calculate new width based on mouse position
          const newWidthPx = calculateWidth(e, startX.current, startWidth.current);
