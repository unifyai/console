...
describe('casing utilities', () => {
  ...
  describe('transformSpecificKeys', () => {
    it('only transforms specified keys', () => {
      const input = { user_id: '123', role_name: 'admin', status: 'active' };
      const result = transformSpecificKeys(
        input,
        ['user_id', 'role_name'],
        snakeToCamel
      );
    ...
    it('leaves unspecified keys unchanged', () => {
      const input = { user_id: '123', already_correct: 'value' };
      const result = transformSpecificKeys(input, ['user_id'], snakeToCamel);
