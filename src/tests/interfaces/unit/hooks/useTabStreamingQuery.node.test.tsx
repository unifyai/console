...
describe('useTabStreamingQuery', () => {
  ...
  it('handles race conditions: ignores slow response from previous tab when switching to new tab', async () => {
    ...
    const tabActions = {
      ...
      getByName: vi.fn(async (projectId, interfaceId, name) =>
        name === 'Slow Tab' ? slowTabData : fastTabData
      ),
    } as unknown as GranularTabActions;
    ...
    const buildCompleteTabData = vi.fn(async (tabId) => {
      if (tabId === 'slow-tab-id') {
        // Sleep 200ms for the slow tab
        await new Promise((resolve) => setTimeout(resolve, 200));
        return {
          tabData: { id: tabId, name: 'Slow Tab', interfaceId: mockInterfaceId } as TabData,
          tiles: [],
          tableTiles: [],
          plotTiles: [],
          fields: [],
      ...
      return {
        tabData: { id: tabId, name: 'Fast Tab', interfaceId: mockInterfaceId } as TabData,
        tiles: [],
        tableTiles: [],
        plotTiles: [],
        fields: [],
    ...
    (baseState as any).interfacesById = {
      ...(baseState.interfacesById || {}),
      [mockInterfaceId]: {
        ...(baseState.interfacesById?.[mockInterfaceId] || {}),
        tabIds: [
          ...((baseState.interfacesById?.[mockInterfaceId] as any)?.tabIds || []),
          slowTab.id,
          fastTab.id,
        ],
        tabNames: [
          ...((baseState.interfacesById?.[mockInterfaceId] as any)?.tabNames || []),
          slowTab.name,
          fastTab.name,
        ],
  ...
  it('prefetchTab() actually calls buildCompleteTabData (not returning null)', async () => {
    ...
    const buildCompleteTabData = vi.fn(async (interfaceId, tabId, tabName, projectId) => {
      if (tabName === 'Tab 2') {
  ...
  it('respects concurrency limit for prefetching', async () => {
    ...
    const buildCompleteTabData = vi.fn(
      async (interfaceId: string, tabId: string, tabName: string) => {
        return {
