...
describe('useTableDataQueryWithTracking', () => {
  ...
  it('initializes with empty data item', async () => {
    ...
    const { result } = renderHook(
      () => useTableDataQueryWithTracking('tile-1', 'tab-1'),
      { wrapper }
    );
  ...
  it('updateLogsByRowIds updates specific log entries', async () => {
    ...
    await waitFor(() => {
        const log1 = result.current.tableData.logs.find((l: any) => l.id === '1');
        const log2 = result.current.tableData.logs.find((l: any) => l.id === '2');
