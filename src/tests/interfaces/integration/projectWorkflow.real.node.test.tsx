...
describe('@real Complete Project Workflow (Real API)', () => {
  ...
  it(
    '@real updates resources in the workflow chain',
    realTestOptionsExtended,
    async () => {
      ...
      if (createdResources.tileId) {
        ...
        expect(updatedTile.color).toBe('#0000ff');
      }
    }
  );

  it(
    '@real deletes resources in the workflow chain',
    realTestOptionsExtended,
    async () => {
      ...
      expect(projectDeleteResult.info).toContain('deleted');
    }
  );
});
