...
describe('@real Infinite Logs Pagination (Real API)', () => {
  ...
  beforeAll(async () => {
    ...
    const logs = Array.from({ length: 15 }, (_, i) => ({
      entries: { message: `Test log ${i + 1}`, index: i + 1 },
    }));
  ...
  it(
    '@real fetches first page with correct limit',
    realTestOptionsExtended,
    async () => {
  ...
  it(
    '@real fetches second page with correct offset',
    realTestOptionsExtended,
    async () => {
      const getLogsFn = await getLogs(TEST_API_KEY);
      ...
      if (
        firstPage.convertedLogs.length > 0 &&
        secondPage.convertedLogs.length > 0
      ) {
  ...
  it(
    '@real handles fetching beyond available logs',
    realTestOptionsExtended,
    async () => {
      const getLogsFn = await getLogs(TEST_API_KEY);
  ...
  it(
    '@real totalCount remains consistent across pages',
    realTestOptionsExtended,
    async () => {
      const getLogsFn = await getLogs(TEST_API_KEY);
