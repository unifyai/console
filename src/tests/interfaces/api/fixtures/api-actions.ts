...
export const customKeysApi = {
  ...
  async rename(name: string, newName: string): Promise<{ info?: string }> {
    const params = new URLSearchParams({ name, newName: newName });
...
export const customEndpointsApi = {
  ...
  async create(
    ...
    const params = new URLSearchParams({ name, url, keyName: keyName });
    if (modelArg) params.append('modelArg', modelArg);
