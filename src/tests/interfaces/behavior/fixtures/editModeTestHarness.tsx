...
function EditModeInner({
  ...
  return (
    <div data-testid="edit-mode-container" className="min-h-screen bg-gray-50">
      {/* Toolbar */}
      <div className="flex items-center gap-2 border-b bg-white p-4" data-testid="edit-toolbar">
        <button
      ...
      {/* Simulated tile grid */}
      <div className="grid grid-cols-3 gap-4 p-4" data-testid="tile-grid">
        {Array.from({ length: tileCount }, (_, i) => (
          ...
          >
            ...
            <div className={isEditMode ? 'ml-8' : ''}>Tile {i + 1}</div>
            {isEditMode && (
              <button
...
export function renderEditMode(options: EditModeTestOptions = {}): EditModeTestResult {
  ...
  const checkpointStore: React.MutableRefObject<{
    interface: any;
    tab: any;
    tiles: TileSnapshot[];
  } | null> = {
