...
describe('Organization, Role & Team Server Actions (Integration)', { meta: { mock: false } }, () => {
    ...
    describe('Role Actions', () => {
        ...
        it('createRole: should create a custom role in the org', async () => {
            if (!testOrgId || availablePermissions.length === 0) throw new Error("Org Setup or Permissions fetch failed");
        ...
        it('removePermissionFromRole: should remove a permission', async () => {
            ...
            expect(isError(res)).toBe(false);
            if (!isError(res)) {
                const hasPerm = (res.permissions as OrganizationPermission[]).some(p => p.id === permIdToRemove);
