...
export const mockOrgActions = {
  ...
  transferOwnership: vi.fn(async (id, newOwnerId) => ({
    ...mockOrganizations[0],
    ownerId: newOwnerId,
  })),
...
export const mockTeamActions = {
  ...
  updateTeam: vi.fn(async (orgId, teamId, name, description) => ({
    ...mockTeams[0],
    id: teamId,
    name,
    description,
  ...
  getTeamDetails: vi.fn(
    async (orgId, teamId) => mockTeams.find((t) => t.id === teamId) || mockTeams[0]
  ),
};

export const mockRoleActions = {
  getRoles: vi.fn(async () => mockRoles),
  ...
  updateRole: vi.fn(async (orgId, roleId, name, description) => ({
    ...mockRoles[0],
    id: roleId,
    name,
    description,
