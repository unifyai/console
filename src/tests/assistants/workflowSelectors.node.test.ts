...
describe('workflow selectors', () => {
  ...
  it('lists only unconnected requirements as unmet', () => {
    const subject = workflow({
      requirements: [
        { canonicalSlug: 'gmail', displayName: 'Gmail', connected: true },
        { canonicalSlug: 'notion', displayName: 'Notion', connected: false },
        { canonicalSlug: 'web', displayName: 'Web browsing', connected: true, builtin: true },
      ],
    });
  ...
  it('splits recurring tasks from the one-shot provisioning task, case-insensitively', () => {
    const subject = workflow({
      sets: {
        tasks: [
