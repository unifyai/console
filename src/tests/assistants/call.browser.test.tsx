...
describe('Assistant Call', () => {
    ...
    beforeEach(async () => {
        ...
        vi.useRealTimers(); 
        // Reset shared state
        (window as any).__mockLiveKitState = { micEnabled: true, camEnabled: false, screenShareEnabled: false };
        ...
        mockAssistantActions.desktop.getLiveviewUrl = vi.fn().mockResolvedValue({ liveviewUrl: 'https://vnc.example.com' });
    ...
    describe('A-LiveKit Room Connection', () => {
        ...
        it('handles a full successful call lifecycle', 
        { 
            meta: { 
                alias: 'Call-Lifecycle-Success',
                scenario: "User initiates a video call which connects successfully and then hangs up.",
                behavior: "The UI transitions through connecting states, shows the assistant view upon connection, and cleans up upon hangup."
            } 
        }, 
        async () => {
            renderPage();
        ...
        it('handles API failure during connection setup', 
        { 
            meta: { 
                alias: 'Call-API-Failure',
                scenario: "The backend returns an error (e.g., quota exceeded) when requesting connection details.",
                behavior: "The UI retries the connection request up to the limit, then stops and does not attempt to connect to the room."
            } 
        }, 
        async () => {
            vi.useRealTimers();
            ...
            mockAssistantActions.call.getConnectionDetails = vi.fn().mockResolvedValue({ detail: 'Backend Error: Quota Exceeded' });
            ...
            await waitFor(() => expect(screen.queryByText('Setting up a connection...')).toBeNull(), { timeout: 5000 });
            ...
            if (roomConstructor.mock.results.length > 0) {
                const instance = roomConstructor.mock.results[roomConstructor.mock.results.length - 1].value as MockRoom;
                expect(instance.connect).not.toHaveBeenCalled();
            }
        });

        it('handle user hanging up immediately while connecting', 
        { 
            meta: { 
                alias: 'Call-Stress-Rapid-Close',
                scenario: "User clicks hang up immediately while the connection spinner is still visible.",
                behavior: "The connection process aborts immediately, and the room state remains disconnected."
            } 
        }, 
        async () => {
        ...
        it('handles unexpected remote disconnection gracefully', 
        {
            meta: {
                alias: 'Call-Unexpected-Disconnect',
                scenario: "The call is active, but the assistant's connection drops unexpectedly.",
                behavior: "The UI handles the disconnection event by resetting the view to the pre-call state."
            }
        },
        async () => {
            renderPage();
        ...
        async () => {
            ...
            vi.mocked(LiveKitClient.Room).mockImplementationOnce(function() {
                ...
                room.localParticipant.setMicrophoneEnabled = vi.fn().mockRejectedValue(new Error('Permission denied'));
                return room;
            } as any);
        ...
        it('allows retrying connection after a failure', 
        {
            meta: {
                alias: 'Call-Retry-Flow',
                scenario: "The connection times out, and the user clicks the retry button.",
                behavior: "The UI initiates a fresh connection attempt, successfully resetting error states and calling the API again."
            }
        },
        async () => {
        ...
        it('handles rapid connect/disconnect cycles', 
        {
            meta: {
                ...
                scenario: "User rapidly toggles the call button and hangs up before connections finalize.",
                behavior: "The system handles race conditions gracefully, ensuring the final state matches the user's last action."
            }
        },
        async () => {
            renderPage();
        ...
        it('cleans up the Room connection when the component unmounts', 
        {
            meta: {
                alias: 'Call-Unmount-Cleanup',
                scenario: "User navigates away (unmounts component) while a call is active.",
                behavior: "The Room connection is explicitly disconnected to prevent memory leaks or phantom calls."
            }
        },
        async () => {
        ...
        it('prevents starting a second call while one is already active', 
        {
            meta: {
                alias: 'Call-Prevent-Concurrent',
                scenario: "User attempts to call a second assistant while already in a call.",
                behavior: "The second call action is blocked or disabled, and an appropriate warning is shown."
            }
        },
        async () => {
            renderPage();
        ...
        it('resets transient state between separate call sessions', 
        {
            meta: {
                alias: 'Call-State-Reset-On-Close',
                scenario: "User finishes one call, closes it, and immediately starts a new one.",
                behavior: "All transient state (participants, connection status) is reset, ensuring the second call starts fresh."
            }
        },
        async () => {
            renderPage();
        ...
        it('handles network interruptions without crashing', 
        {
            meta: {
                alias: 'Call-Network-Resilience',
                scenario: "The network drops temporarily causing a 'Reconnecting' state.",
                behavior: "The UI maintains the active call view without crashing during the reconnection phase."
            }
        },
        async () => {
            renderPage();
    ...
    describe('B-Assistant Screen Share', () => {
        ...
        it('activates screen share view when remote control is toggled',
        {
            meta: {
                alias: 'Call-ScreenShare-Activation',
                scenario: "User clicks the remote control button while in a call.",
                behavior: "The API is called to get the liveview URL, and an iframe displays the assistant's screen."
            }
        },
        async () => {
            renderPage();
        ...
        it('allows toggling screen share off to return to standard view',
        {
            meta: {
                alias: 'Call-ScreenShare-ToggleOff',
                scenario: "User has screen share active and clicks the button again to hide it.",
                behavior: "The iframe is removed, and the standard assistant avatar/video view is restored."
            }
        },
        async () => {
        ...
        it('toggles interactive mode sending correct system events',
        {
            meta: {
                alias: 'Call-ScreenShare-Interaction',
                scenario: "User toggles interactive mode on the screen share view.",
                behavior: "System events for 'pause_actor' (enable user control) and 'resume_actor' (return control to AI) are sent to the backend."
            }
        },
        async () => {
            renderPage();
        ...
        it('handles API failure when toggling interactive mode',
        {
            meta: {
                alias: 'Call-ScreenShare-Interaction-Failure',
                scenario: "User tries to enable interactive mode but the API fails.",
                behavior: "The interactive mode state does not change, and an error notification is displayed."
            }
        },
        async () => {
             mockAssistantActions.desktop.sendSystemEvent = vi.fn().mockResolvedValue({ detail: 'Failed to send event' });
        ...
        it('resets interactive mode when screen share is closed and reopened',
        {
            meta: {
        ...
        it('handles failure to retrieve liveview url gracefully',
        {
            meta: {
                alias: 'Call-ScreenShare-Error',
                scenario: "The backend fails to return a valid liveview URL when screen share is requested.",
                behavior: "An error notification is shown, and the view does not switch to the iframe."
            }
        },
        async () => {
            mockAssistantActions.desktop.getLiveviewUrl = vi.fn().mockRejectedValue(new Error('Failed to fetch URL'));
        ...
        it('cleans up screen share state when call is hung up while active',
        {
            meta: {
                alias: 'Call-ScreenShare-Hangup-While-Active',
                scenario: "User hangs up the call while viewing the assistant's screen.",
                behavior: "The call disconnects cleanly, and screen share state is reset for future calls."
            }
        },
        async () => {
             renderPage();
    ...
    describe('C-View States (Fullscreen & Minimized)', () => {
        ...
        it('minimizes call to a floating widget',
        {
            meta: {
                alias: 'Call-View-Minimize',
                scenario: "User clicks the minimize button in the main dialog.",
                behavior: "The main call dialog disappears, and a small floating widget appears in the bottom right."
            }
        },
        async () => {
            renderPage();
        ...
        it('expands call from floating widget back to dialog',
        {
            meta: {
        ...
        it('hangs up directly from the minimized widget',
        {
            meta: {
        ...
        it('transfers call to new tab (Pop-out)', 
        {
            meta: {
                alias: 'Call-View-PopOut',
                scenario: "User clicks the 'Open in new tab' button.",
                behavior: "Calls window.open and transfers call data to local storage to hand off the session."
            }
        },
        async () => {
