 ...
 *
 * Two properties carry the weight. The question is asked with the *caller's own*
 * key, so Orchestra reports their real scope and the answer cannot be widened by
 * asking differently. And the org-wide listing is tried first: without
 ...
 * while the shell's own roster shows it to them.
 */
...
describe('assistantField', () => {
  ...
  it('stringifies so a numeric id compares against a path segment', () => {
    expect(assistantField({ agent_id: 4211 }, 'agent_id', 'agentId')).toBe('4211');
  });

  it('returns null for an absent or null field rather than the string "null"', () => {
