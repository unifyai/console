...
describe('Assistant Profile Chat', () => {
    ...
    describe('D - Chat History', () => {
        ...
        it('prevents stale transcript fetches from overwriting user messages (Zombie Request)', { 
            meta: { 
                alias: 'History-Zombie-Request',
                scenario: 'Component remounts (simulating StrictMode or nav), triggering 2nd fetch. 1st fetch resolves late.',
                behavior: 'Late resolution of 1st fetch is ignored via cleanup function.'
            } 
        }, async () => {
            let resolveStale: (val: any) => void;
            let resolveFresh: (val: any) => void;

            // Mock getTranscripts to allow manual resolution
            const getTranscriptsMock = vi.fn()
                .mockReturnValueOnce(new Promise(r => { resolveStale = r; })) // 1st call (Zombie)
                .mockReturnValueOnce(new Promise(r => { resolveFresh = r; })); // 2nd call (Fresh)

            const actionsOverride = {
                chat: {
                    getTranscripts: getTranscriptsMock,
                    message: vi.fn().mockResolvedValue({}),
                    updateTranscripts: vi.fn().mockResolvedValue({})
                }
            };

            // 1. Initial Render (Trigger Fetch 1)
            const { rerender } = render(
                <ChatTestWrapper 
                    panelKey="mount-1"
                    initialHistory={undefined} // Force fetch
            ...
            // 2. Remount Component (Trigger Fetch 2)
            // Changing panelKey forces unmount (triggering cleanup of Fetch 1) and remount (starting Fetch 2)
            rerender(
                <ChatTestWrapper 
                    panelKey="mount-2"
                    initialHistory={undefined}
            ...
            // SIMULATE NEW CONNECTION OPEN - This is crucial!
            // The re-render creates a new EventSource which starts in 'connecting' state.
            // The input remains disabled until we signal the connection is open.
            act(() => {
                mockEventSourceInstance!.simulateOpen();
            });

            // 3. Resolve Fetch 2 (Fresh) immediately with empty history
            // This simulates the successful load of the *current* session
            await act(async () => {
                resolveFresh([]); 
            });

            // 4. Wait for input to unlock (indicates loading finished for current session)
            const input = await screen.findByPlaceholderText('Send a message...');
            expect(input).not.toBeDisabled();

            // 5. User types and sends message
            // State should now contain: ["My New Message"]
            const user = userEvent.setup();
            await user.type(input, 'My New Message');
            await user.keyboard('{Enter}');
            expect(screen.getByText('My New Message')).toBeInTheDocument();

            // 6. NOW resolve Fetch 1 (Zombie) with different history
            // If the bug exists, this "old" fetch result will overwrite the state, removing "My New Message".
            await act(async () => {
                resolveStale([{ id: 'old-1', role: 'assistant', content: 'Old Zombie Content', timestamp: new Date() }]);
            });

            // 7. Assert "My New Message" is still there and "Old Zombie Content" is ignored
            expect(screen.getByText('My New Message')).toBeInTheDocument();
            expect(screen.queryByText('Old Zombie Content')).not.toBeInTheDocument();
        });
    });
