...
export const mockAssistantActions: any = {
  ...
  desktop: {
    getLiveviewUrl: vi.fn(
      async (_assistantId: string, _ownerId: string, _organizationId: number | null) => ({
        liveviewUrl: 'https://vnc.example.com',
      })
    ),
    buildLiveviewUrl: vi.fn(
      async (rawUrl: string, _ownerId: string, _organizationId: number | null) => ({
