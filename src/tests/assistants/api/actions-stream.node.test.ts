...
function makePubSubMessage(payload: Record<string, unknown>, ackId = 'ack-1') {
  const encoded = Buffer.from(JSON.stringify({ event: payload })).toString('base64');
  return {
    ackId,
    message: { data: encoded, messageId: 'msg-1', publishTime: new Date().toISOString() },
  };
}
...
function createPubSubMock(
  ...
  return async (reqOpts: { url: string; method: string; data?: any }) => {
    ...
    if (reqOpts.method === 'POST' && reqOpts.url.endsWith(':pull')) {
      ...
      // No more batches — abort and return empty so the loop terminates cleanly
      controller.abort();
...
/** Read all SSE data chunks from a streaming Response until the stream ends. */
...
describe('Actions SSE Stream Route', () => {
  ...
  it('creates a unique ephemeral subscription per connection', async () => {
    ...
    // Two separate controllers so each connection terminates independently
    ...
    mockRequest.mockImplementation(async (opts: { url: string; method: string; data?: any }) => {
      ...
      if (opts.method === 'POST' && opts.url.endsWith(':pull')) {
        // Abort both connections after subscription creation is verified
        ctrl1.abort();
        ctrl2.abort();
        return { status: 200, data: { receivedMessages: [] } };
      }
    ...
    // Two separate subscriptions must have been created
    expect(createdSubscriptions.length).toBe(2);
    expect(createdSubscriptions[0]).not.toBe(createdSubscriptions[1]);

    // Both subscription names should reference the assistant and use the -sse- infix
    for (const url of createdSubscriptions) {
      expect(url).toContain(TEST_ASSISTANT_ID);
      expect(url).toContain('-actions-sse-');
    }

    // Drain both streams to allow cleanup
  ...
  it('deletes the ephemeral subscription when the stream ends', async () => {
    ...
    // The stream cleanup should have deleted the subscription
  ...
  it('filters out excluded managers', async () => {
    ...
    // Only ContactManager should come through (MemoryManager is excluded)
  ...
  it('uses staging suffix in topic name when ORCHESTRA_URL contains staging', async () => {
    ...
    expect(createPayload).toBeTruthy();
    expect(createPayload.topic).toContain('-staging');
    expect(createPayload.topic).toBe(
