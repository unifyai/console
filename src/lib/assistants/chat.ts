...
export const triggerContactSync = async () => {
  return async (assistantId: string): Promise<ResponseProps> => {
    ...
    if (!ADMIN_KEY) {
      console.error(
        '[triggerContactSync] Server configuration error: ORCHESTRA_ADMIN_KEY is not set.'
      );
    ...
    try {
      ...
      if (!webhookResponse.ok) {
        ...
        console.error(
          `[triggerContactSync] Webhook error (${webhookResponse.status}): ${errorText}`
        );
        return { detail: `Failed to trigger contact sync: ${errorText}` };
      }

      console.log(
        `[triggerContactSync] Successfully triggered contact sync for assistant ${assistantId}`
      );
