...
import { requireAdminKey, requireUserApiKey } from '@/lib/server-action-session';
...
export async function listAvailablePhoneCountries(): Promise<AvailablePhoneCountry[]> {
  const apiKey = await requireAdminKey();
  try {
...
export async function listAvailableSocialPlatforms(): Promise<
  AvailableSocialPlatform[] | ResponseProps
> {
  const apiKey = await requireAdminKey();
  try {
    const response = await fetch(
    ...
    if (!response.ok) {
      return {
        detail: data.detail || `Failed to list available social platforms: ${response.statusText}`,
      };
    }
...
export async function deleteAssistantContact(
  assistantId: string,
 ...
 */
export async function createAssistantContact(
  assistantId: string,
 ...
 */
export async function connectAssistantAccount(
  assistantId: string,
 ...
 */
export async function disconnectAssistantAccount(assistantId: string): Promise<ResponseProps> {
  const apiKey = await requireUserApiKey();
  try {
    const response = await fetch(
      `${getInternalApiBaseUrl()}/api/assistant/${assistantId}/connect`,
...
export async function getGrantedFeatures(
  assistantId: string
): Promise<GrantedFeaturesResponse | ResponseProps> {
  const apiKey = await requireUserApiKey();
  try {
    const response = await fetch(
 ...
 */
export async function fetchContactCosts(): Promise<ContactCosts | ResponseProps> {
  try {
