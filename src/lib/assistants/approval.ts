...
export const fetchCurrentUserHiringProfile = async () => {
  return async (): Promise<HiringProfileData | ResponseProps> => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Unknown error fetching hiring profile.';
  ...
  return async (): Promise<AssistantHiringApprovalResponse> => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXTAUTH_URL}/api/user/assistant-hiring-approval`,
        {
          method: 'POST',
      ...
      if (!response.ok) {
        return {
          message: data.detail || `Failed to request access: ${response.statusText}`,
          detail: data.detail,
        };
      }
...
export const claimAssistantHiringToken = async (apiKey: string) => {
  return async (token: string): Promise<AssistantHiringApprovalResponse> => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXTAUTH_URL}/api/user/claim-assistant-hiring-one-time-link`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', apiKey },
      ...
      if (!response.ok) {
        return {
          message: data.detail || `Failed to claim token: ${response.statusText}`,
          detail: data.detail,
        };
      }
      return data as AssistantHiringApprovalResponse;
