 ...
 */
export async function listDemoAssistants(): Promise<DemoAssistant[] | ResponseProps> {
  ...
  try {
    ...
    if (!response.ok) {
      const errorMessage = data.detail || `Failed to list demo assistants: ${response.statusText}`;
 ...
 */
export async function createDemoAssistant(
  payload: DemoAssistantCreatePayload
): Promise<DemoAssistant | ResponseProps> {
  const apiKey = await requireUserApiKey();
  try {
 ...
 */
export async function getDemoMeta(demoId: number): Promise<DemoAssistantMeta | ResponseProps> {
  const apiKey = await requireUserApiKey();
  try {
    const response = await fetch(`${process.env.NEXTAUTH_URL}/api/demo/assistant/${demoId}/meta`, {
      method: 'GET',
      headers: { apiKey: apiKey },
    });
    ...
    try {
      ...
      console.error(`[demo/assistant.ts getDemoMeta] Failed to parse JSON response ${parseError}`);
      return { detail: 'Received an invalid response from the server.' };
    }
 ...
 */
export async function listDemoMeta(): Promise<DemoAssistantMeta[] | ResponseProps> {
  const apiKey = await requireUserApiKey();
  try {
    ...
    try {
      ...
      console.error(`[demo/assistant.ts listDemoMeta] Failed to parse JSON response ${parseError}`);
      return { detail: 'Received an invalid response from the server.' };
    }
...
export async function getDemoContacts(
  ownerId: string,
  assistantId: string
): Promise<DemoContact[] | ResponseProps> {
  const apiKey = await requireUserApiKey();
  try {
 ...
 */
export async function getDemoSpending(
  assistantId: string
 ...
 */
export async function deleteDemoAssistant(
  assistantId: string
): Promise<{ success: boolean } | ResponseProps> {
  const apiKey = await requireUserApiKey();
  try {
    ...
    if (!response.ok) {
      const errorMessage = data.detail || `Failed to delete demo assistant: ${response.statusText}`;
      return { detail: errorMessage };
    }
