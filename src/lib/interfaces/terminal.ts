...
export const getDevbox = async (apiKey: string, userId: string) => {
  return async () => {
    ...
    const response = await fetch(`${process.env.NEXTAUTH_URL}/api/devbox?userId=${userId}`, {
      method: 'GET',
...
export const createDevbox = async (apiKey: string, userId: string) => {
  return async () => {
    ...
    const response = await fetch(`${process.env.NEXTAUTH_URL}/api/devbox`, {
      method: 'POST',
      headers: { apiKey: apiKey },
    ...
    const response = await fetch(`${process.env.NEXTAUTH_URL}/api/code`, {
      ...
      body: JSON.stringify({
        ...
        env: Array.isArray(env)
          ? Object.fromEntries(
              (env as { key: string; value: string }[]).map(({ key, value }) => [key, value])
            )
          : env,
      }),
    });
...
export const getCodeOutput = async (apiKey: string) => {
  return async (filePath: string) => {
    ...
    const response = await fetch(`${process.env.NEXTAUTH_URL}/api/code?filePath=${filePath}`, {
      method: 'GET',
      headers: { apiKey: apiKey },
    });
...
export const createTerminalSession = async (apiKey: string, userId: string) => {
  return async (shell: string = 'bash', cwd: string = '/project/sandbox') => {
    'use server';

    const response = await fetch(`${process.env.NEXTAUTH_URL}/api/code/terminal`, {
      method: 'POST',
      headers: { apiKey: apiKey },
      body: JSON.stringify({
        userId: userId,
...
export const runTerminalCommand = async (apiKey: string) => {
  return async (sessionId: string, command: string) => {
    'use server';
