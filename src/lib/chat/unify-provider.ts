...
export function createUnifyProvider(options: UnifyProviderSettings = {}): UnifyProvider {
  ...
  const createChatModel = (modelId: UnifyChatModelId, settings: UnifyChatSettings = {}) =>
    new UnifyChatLanguageModel(modelId, settings, {
      provider: 'unify.chat',
      baseURL,
      headers: getHeaders,
      fetch: options.fetch,
  ...
  const provider = function (modelId: UnifyChatModelId, settings?: UnifyChatSettings) {
    if (new.target) {
