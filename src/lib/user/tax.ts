...
import { ValidateTaxIdRequest, TaxIdValidationResponse, SupportedTaxCountriesResponse } from '@/types/user';
...
export async function getSupportedTaxCountries(apiKey: string): Promise<SupportedTaxCountriesResponse> {
  const orchestraClient = await getOrchestraUserClient(apiKey);
