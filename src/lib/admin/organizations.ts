...
export async function getOrganizationDetailAction(
  orgId: number
): Promise<AdminOrgDetail | ResponseProps> {
  const denied = await requireUnifyAdmin();
  if (denied) return denied;
  ...
  const verification =
    verificationResult && !('detail' in verificationResult)
      ...
      : {};
  const billing =
    billingResult && !('detail' in billingResult) ? (billingResult as Record<string, unknown>) : {};
...
export async function lookupUserByEmailAction(
  email: string
): Promise<AdminUserLookup | ResponseProps> {
  const denied = await requireUnifyAdmin();
  if (denied) return denied;
  return safeFetch(
...
export async function listOrgInvitesAction(
  orgId: number
): Promise<AdminOrgInvite[] | ResponseProps> {
  const denied = await requireUnifyAdmin();
  if (denied) return denied;
...
export async function enableFreeTrialAction(orgId: number): Promise<ResponseProps> {
  const denied = await requireUnifyAdmin();
  if (denied) return denied;
  return safeFetch(
...
export async function disableFreeTrialAction(orgId: number): Promise<ResponseProps> {
  const denied = await requireUnifyAdmin();
  if (denied) return denied;
  return safeFetch(
    `${backendUrl}/admin/organization/${orgId}/free-trial`,
...
export async function verifyOrganizationAction(orgId: number): Promise<ResponseProps> {
  const denied = await requireUnifyAdmin();
  if (denied) return denied;
  return safeFetch(
...
export async function unverifyOrganizationAction(orgId: number): Promise<ResponseProps> {
  const denied = await requireUnifyAdmin();
  if (denied) return denied;
  return safeFetch(
    `${backendUrl}/admin/organization/${orgId}/verify`,
...
export async function freezeAccountAction(orgId: number, freeze: boolean): Promise<ResponseProps> {
  const denied = await requireUnifyAdmin();
  if (denied) return denied;
  return safeFetch(
