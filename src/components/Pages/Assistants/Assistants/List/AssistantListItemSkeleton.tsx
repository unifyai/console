...
export function AssistantListItemSkeleton({ isFolded }: { isFolded?: boolean }) {
    if (isFolded) {
        return <Skeleton role="list-item-skeleton" className="h-8 w-8 rounded-full flex-shrink-0 bg-muted" />;
    }

    return (
        <div role="list-item-skeleton" className={cn("flex items-center justify-between p-2 rounded-md")}>
