...
export function AssistantCommunicationTranscriptionsPanel({ userImage, assistantPhoto }: AssistantCommunicationTranscriptionsPanelProps) {
    const transcriptions = useTranscriptions();
    const { localParticipant } = useLocalParticipant();
    const participants = useParticipants();

    const sortedTranscriptions = React.useMemo(() => {
        return transcriptions.sort((a, b) => (a.streamInfo?.startTime ?? 0) - (b.streamInfo?.startTime ?? 0));
    }, [transcriptions]);
                ...
                <div className="p-4 space-y-4">
                    {sortedTranscriptions.length === 0 ? (
                        ...
                        sortedTranscriptions.map((transcription, index) => {
                            ...
                            const fallback = participantName?.substring(0, 2).toUpperCase() || (isUser ? 'U' : 'A');
                            
                            const showName = index === 0 || sortedTranscriptions[index-1].participantInfo.identity !== transcription.participantInfo.identity;

                            return (
                                <div key={`${transcription.streamInfo.startTime}-${index}`} className="flex items-start gap-3">
