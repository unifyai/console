...
const sharingCopy = [
  "By default, all skills acquired and knowledge retained are personal to each user's own private teammate, with no ability for each teammate to share knowledge, skills, or know-how. Each user's teammate learns in isolation, based on that user alone. If you'd like the teammates to also have the option to read/write from a shared pool across the org where appropriate, select shared below.",
  'If you want more granular control, then it is best to set up dedicated teams. Users and teammates can then be assigned to teams to enable controlled sharing within specific teams, rather than blanket org-wide sharing. Selecting shared below effectively creates a new Org team and adds every person and teammate to this team automatically.',
  'Even in shared mode, each teammate will still carefully decide what is useful to share and what is best to keep personal. Transcripts, emails, and files are never shared. Only knowledge, skills, and general know-how are optionally shared to create a faster hive-mind approach to learning across the team.',
];
...
const options: Array<{
...
}> = [
  {
    value: 'private',
    title: 'Private',
    description: "Each user's teammate learns from that user alone.",
  },
  {
    value: 'shared',
    title: 'Shared',
