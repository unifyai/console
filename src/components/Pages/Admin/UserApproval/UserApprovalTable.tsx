...
import {
  UserApprovalEntry,
  AssistantHiringApprovalAction,
  ASSISTANT_HIRING_APPROVAL_ACTIONS,
  ASSISTANT_HIRING_APPROVAL_DISPLAY,
} from '@/types/admin';
import {
  MoreHorizontal,
  Loader2,
...
const VirtuosoShadcnTable: TableComponents<UserApprovalEntry>['Table'] = React.forwardRef(
  ({ style, children, ...props }, ref) => {
    return (
      <Table ref={ref as React.Ref<HTMLTableElement>} style={style} {...props} className="w-full">
        {children}
      </Table>
    );
  }
...
export function UserApprovalTable({
  ...
  const [confirmAction, setConfirmAction] = React.useState<{
    userId: string;
    newStatus: AssistantHiringApprovalAction;
    userName: string;
  } | null>(null);
      ...
      <Table className="w-full">
        <thead>
          <TableRow>
            ...
            <TableHead className="w-[20%] min-w-[120px] text-center">Current Status</TableHead>
      ...
      <TableVirtuoso
        ...
        itemContent={(_index, user) => {
          if (user.id === LOADING_MORE_USERS_ID) {
            return (
              <TableCell colSpan={4} className="h-[57px] p-4 text-center">
          ...
          return (
            <>
              <TableCell className="truncate font-medium" title={user.name || user.email}>
              ...
              <TableCell className="text-center">
                <Badge
                  variant="outline"
                  className={cn(
                    'whitespace-nowrap px-2 py-0.5 text-xs capitalize',
                    statusColors[currentStatusKey] || statusColors.none
                  )}
                >
                  <StatusIcon className="mr-1.5 h-3 w-3" />
                  {ASSISTANT_HIRING_APPROVAL_DISPLAY[
                    currentStatusKey as keyof typeof ASSISTANT_HIRING_APPROVAL_DISPLAY
                  ] ||
                    user.assistantHiringApproval ||
                    'None'}
                </Badge>
              </TableCell>
              <TableCell className="text-right">
                    ...
                    <DropdownMenuContent align="end">
                      {availableActions.map((action) => (
                        <DropdownMenuItem
                          key={action}
                          onClick={() =>
                            openConfirmationDialog(user.id, action, user.email, user.name)
                          }
                          disabled={user.assistantHiringApproval === action}
                          className={cn(
                            user.assistantHiringApproval === action &&
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Are you sure you want to set the assistant hiring approval status for{' '}
                <strong>{confirmAction.userName}</strong> to{' '}
                <strong>{ASSISTANT_HIRING_APPROVAL_DISPLAY[confirmAction.newStatus]}</strong>?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel onClick={() => setConfirmAction(null)}>Cancel</AlertDialogCancel>
              <AlertDialogAction
                onClick={() => handleUpdate(confirmAction.userId, confirmAction.newStatus)}
              >
                Confirm
              </AlertDialogAction>
            </AlertDialogFooter>
