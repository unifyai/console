...
const TaxClassificationForm = forwardRef<TaxClassificationFormHandle, TaxClassificationFormProps>(
  ({ onSubmit, onValidationChange, onCancel, isLoading, error, initialData }, ref) => {
      ...
      const fetchSupportedCountries = async () => {
        try {
          ...
          if (response.ok) {
            ...
            const countries: TaxCountry[] = Object.entries(data.supportedCountries)
          ...
          try {
            ...
            if (response.ok) {
              ...
              try {
                ...
                const message =
                  errorData?.detail?.[0]?.msg || errorData?.detail || 'Invalid tax ID';
            ...
            setTaxIdValidation({
              valid: false,
              errorMessage: 'Unable to validate tax ID. Please check the format.',
            });
          } finally {
    ...
    const handleAddressChange = (
      field: keyof TaxClassificationFormData['businessAddress'],
      value: string
    ) => {
      setFormData((prev) => ({
        ...prev,
    ...
    useEffect(() => {
      if (
        ...
        const code = countries.getAlpha2Code(formData.businessAddress.country, 'en');
        if (code) {
    ...
    const selectedCountry = supportedCountries.find(
      (c: TaxCountry) => c.code === formData.taxCountry
    );
