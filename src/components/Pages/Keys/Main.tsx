...
const Main = ({
  ...
  const supportedProviderKeys = useMemo(
    () => customKeys.filter((key) => providers.includes(path.basename(key.path))) ?? [],
    [providers, customKeys]
  );
  ...
  const section = (title: string, description: string, form: ReactNode, content: ReactNode) => (
    <div
      className={`flex w-full flex-col gap-10 tutorial-${title.split(' ').join('-').toLowerCase()}`}
    >
      ...
      <div className="flex w-full flex-col gap-5 pb-5 md:flex-row">{content}</div>
    </div>
  );
  ...
  return (
    <SinglePaneBody
      isPending={false}
      body={
        <div className="flex h-full w-full flex-col gap-10 p-5 text-lg font-normal">
          ...
          {section(
            ...
            <CreateKey
              creationFunction={customKeyActions.create}
              choices={providers}
              paths={supportedProviderKeys.map((key) => key.path)}
            />,
            <CustomKeysTable
              customKeyActions={customKeyActions}
              customKeys={supportedProviderKeys}
            />
          )}
          {section(
            'Custom Keys',
            ...
            <CreateKey
              creationFunction={customKeyActions.create}
              choices={providers}
              paths={unsupportedProviderKeys.map((key) => key.path)}
            />,
