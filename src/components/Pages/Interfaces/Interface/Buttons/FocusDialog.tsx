...
const FocusDialog = ({
  ...
  const handleAddSplit = () => {
    ...
    const remainingTiles = tiles.filter(
      (t) => !focusedTiles.map((ft) => ft?.name).includes(t.name)
    );
  ...
  const TileSlot = (tile: Tile | undefined, idx: number) =>
    tile ? (
      <div className="relative flex h-full w-full flex-col overflow-hidden">
        ...
        <div className="relative isolate h-full w-full overflow-hidden p-2">
          <TileCard
            tileId={tile.id}
        ...
        {availableTiles.length > 0 ? (
          ...
          >
            {availableTiles.map((t, i) => (
              <DropdownMenuItem
                key={i}
                onSelect={() => handleSelectTile(t, idx)}
                className="flex w-64 items-center justify-between"
              >
                <span>{t.name}</span>
                {t.type ? icons[t.type as keyof typeof icons] : ''}
              </DropdownMenuItem>
            ))}
          </BaseDropdown>
        ) : (
  ...
  const PanelContent = (tile: Tile | undefined, idx: number) => (
    <div className="h-full w-full overflow-hidden duration-300 animate-in fade-in-0 zoom-in-95">
      {TileSlot(tile, idx)}
    </div>
  );
  ...
  const renderPanels = () => {
    ...
    if (count === 1)
      return (
        <div className="h-full w-full overflow-hidden">{PanelContent(focusedTiles[0], 0)}</div>
      );
    if (count === 2)
      return (
        <ResizablePanelGroup direction="horizontal" className="h-full min-h-0 w-full min-w-0 gap-1">
    ...
    if (count >= 4)
      return (
        <ResizablePanelGroup direction="vertical" className="h-full min-h-0 w-full min-w-0 gap-1">
          <ResizablePanel minSize={20}>
  ...
  return (
    <div className="box-border flex h-full w-full flex-col">
      {/* Toolbar */}
      <div className="bg-background/80 flex items-center gap-4 border-b p-2">
      ...
      {/* Content */}
      <div className="min-h-0 flex-1 overflow-hidden">{renderPanels()}</div>
    </div>
  );
};
