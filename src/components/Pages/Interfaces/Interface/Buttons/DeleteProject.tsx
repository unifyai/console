    ...
    <div>
      ...
      {!success && !error && (
        <RadioGroup
          ...
          onValueChange={(value: DeleteOption) => setSelectedOption(value)}
          className="mt-4 space-y-2"
        >
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="project" id="delete-project" className="p-0.5 pr-0.5" />
            <Label htmlFor="delete-project" className="pt-0.5">
              Delete the entire project
            </Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="logs" id="delete-logs" className="p-0.5 pr-0.5" />
            <Label htmlFor="delete-logs" className="pt-0.5">
              Delete all logs in the project
            </Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem
              value="logsAndContexts"
              id="delete-logs-contexts"
              className="p-0.5 pr-0.5"
            />
            <Label htmlFor="delete-logs-contexts" className="pt-0.5">
              Delete all logs and contexts in the project
            </Label>
          </div>
        </RadioGroup>
      )}
