...
import {
  ...
  ChevronsUpDown,
  Check,
} from 'lucide-react'; // Added ChevronsUpDown, Check
...
export function FileUpload({
  ...
  return (
    <BaseDialog
      open={open}
      setOpen={setOpen}
      className="flex max-h-[90vh] flex-col sm:max-w-[75vw]" // Increased width slightly
      button={
        <ActionButton
      ...
      body={
        <>
          ...
          <div className="flex flex-1 flex-col gap-4 overflow-hidden py-4">
            ...
            {!file && (
              <div
                {...getRootProps()}
                className={cn(
                  'border-muted-foreground/25 hover:bg-muted/25 group relative grid flex-1 cursor-pointer place-items-center rounded-lg border-2 border-dashed px-5 py-10 text-center transition', // Increased py
              ...
              >
                ...
                <div className="flex flex-col items-center justify-center gap-4 sm:px-5">
                  ...
                  <div className="text-body text-muted-foreground/70 flex flex-col justify-start gap-1">
                    <span>Supported:</span>
                    <ul className="ml-2">
            ...
            {file && parsedData && (
              <div className="flex flex-1 flex-col gap-4 overflow-hidden">
                ...
                <div className="bg-muted/50 flex items-center gap-2 rounded-md border p-2">
                  ...
                  <span className="text-body text-strong flex-1 truncate" title={file.name}>
                    {file.name}
                  </span>
                  <Button
                    variant="ghost"
                ...
                <div className="flex flex-1 gap-4 overflow-hidden">
                  {/* Column Switches */}
                  <div className="flex max-w-[220px] flex-col gap-3 overflow-y-auto rounded-md border p-3">
                    {' '}
                    ...
                    {parsedData.headers.map((header) => (
                      <div key={header} className="flex items-center justify-between gap-2">
                        <Label
                          ...
                          className="text-body flex-1 cursor-pointer truncate"
                          title={header}
                        >
                          {header}
                        </Label>
                  ...
                  <div className="flex flex-1 flex-col overflow-hidden rounded-md border">
                    <p className="text-title border-b p-3">
                    ...
                    <ScrollArea className="h-[300px] flex-1">
                      {' '}
                      {/* Adjust h-[...] as needed for ~5 rows visibility */}
                    ...
                    {parsedData.rows.length > MAX_PREVIEW_ROWS && (
                      <p className="text-caption border-t p-2 text-center text-muted-foreground">
          ...
          {/* Error Display Area  */}
          {error && (
            <div className="bg-destructive/10 border-destructive/30 mb-4 flex items-center gap-2 rounded-md border p-3 text-destructive">
              {' '}
              {/* Added mb-4 */}
          ...
          <div className="flex items-end gap-2">
            <div className="flex-1">
                    ...
                    <CommandList>
                      <CommandEmpty>
                        {contextInputValue.trim() ? (
                          <div className="text-body py-6 text-center">
                            {`No context found. Type to create "${contextInputValue}".`}
                          </div>
                        ) : (
                          <div className="text-body py-6 text-center">No contexts found.</div>
                        )}
                      </CommandEmpty>
                      <CommandGroup>
                        {contexts.map((ctx) => (
                          <CommandItem
                            ...
                            onSelect={(currentValue) => {
                              setContextInputValue(
                                currentValue === contextInputValue ? '' : currentValue
                              );
              ...
              {!project && (
                <p className="text-caption mt-1 text-destructive">Select a project first.</p>
              )}
            </div>
          </div>
        </>
