...
const InterfaceTabs = ({ 
  ...
  pendingTabChange,
    ...
    const { meta: tabMetaState, data: tabDataState, ui: tabUIState, uiActions: tabUIActions, dataActions: tabDataActions } = useTab(tabIdOrName || "", interfaceId);
    ...
    const handleContextChange = (ctx: string) => {
        if (syncedTabDataActions && tabUIActions && project && tabName) {
            ...
            syncedTabDataActions.setGlobalContext(ctx, (pending: boolean) => tabUIActions.setDataPending(pending));
    ...
    useEffect(() => {
        if (globalContextOpen || deleteTabOpen || renameTabOpen)
                        ...
                        {tabNamesToShow.map((tabNameToShow, idx) => {
                            return (
                                <TabsTrigger
                                    ...
                                    key={idx}
                                    value={tabNameToShow}
                                    className="relative flex flex-row gap-2 text-muted-foreground hover:text-foreground data-[state=active]:text-primary data-[state=active]:text-strong data-[state=active]:bg-transparent hover:bg-transparent transition-colors"
                            ...
                            <div className="w-56 flex flex-col items-center p-2">
                                ...
                                <div className="w-full border-b pb-1">
                                    <ActionButton
                                        ...
                                        text="Save tab"
                                        tooltip={!project ? "Select a project first" : "Save changes to the current tab"}
                                        icon={saveIcon}
                                        variant="ghost"
                                        disabled={isDisabled}
                                ...
                                <div className="w-full border-b py-1">
                                    <ActionButton
                                        className="w-full justify-start"
                                        text="Reset tab"
                                        tooltip={!project ? "Select a project first" : "Revert current tab changes to the last saved state"}
                                        icon={resetIcon}
                                        variant="ghost"
                                        ...
                                        body={
                                            <div className="space-y-4">
                                                <div>
                                                    ...
                                                    />
                                                    {renameError && (
                                                        <p className="text-caption text-destructive mt-1">{renameError}</p>
                                                    )}
                                                </div>
                                            </div>
                                        }
