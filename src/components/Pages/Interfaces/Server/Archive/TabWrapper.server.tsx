...
export default async function TabWrapper({
  ...
  if (!project) {
    return <div className="flex h-full items-center justify-center">Please select a project</div>;
  }

  // Handle case where no interface is provided
  if (!interfaceId) {
    return (
      <div className="flex h-full items-center justify-center">Please select an interface</div>
    );
  }
  ...
  if (!activeTab) {
    return <div className="flex h-full items-center justify-center">No tabs found</div>;
  }
  ...
  const fields: LogFieldsResponseProps[] = tableTiles.map(
    (tile) => qc.getQueryData(['fields', project, tile.context ?? null]) as LogFieldsResponseProps
  );
