...
const Editor = ({
  ...
  const initialFileName =
    editorTileState?.fileType === 'env' && editorTileState?.fileName === ''
      ? ''
  ...
  useEffect(() => {
    if (!pending && output !== '') setComplete(true);
  }, [pending, output]);

  useEffect(() => {
    if (complete) setTimeout(() => setComplete(false), 5000);
  }, [complete]);
  ...
  const updateEnvValue = async (key: string, newVal: string) => {
    const newLines = envVars.map((ev) =>
      ev.key === key ? `${key}=${newVal}` : `${ev.key}=${ev.value}`
    );
  ...
  const updateEnvKey = async (oldKey: string, newKey: string) => {
    ...
    const newLines = envVars.map((ev) =>
      ev.key === oldKey ? `${newKey}=${ev.value}` : `${ev.key}=${ev.value}`
    );
    const newContent = newLines.join('\n');
    setTempCode(newContent);
    editorTileActions?.setContent(newContent);
  ...
  return (
    <div className="flex h-full w-full flex-col">
      <div className="text-body my-2 ml-4 flex flex-row items-center gap-1">
        ...
        <div className="mr-2 flex flex-row gap-1">
          ...
          <FileDirectory
            ...
            renamingFunction={async (oldPath, newPath) => {
              return { info: 'Rename not implemented' };
            }}
            onOpen={() => {
          ...
          {!isEditMode && !focusPaneOpen && (
            <ActionButton
              ...
              variant={
                focusPaneOpen &&
                (tabUIState?.focusedTileNames || [undefined, undefined]).includes(
                  tileMetaState?.name
                )
                  ? 'primary'
                  : 'outline'
        ...
        {saved && <div className="text-body text-strong text-primary">File saved!</div>}
      </div>
      {tempFileType !== 'env' ? (
        <CodeBlock
          code={tempCode}
          ...
          onRun={async (code: string) => {
            ...
            try {
              ...
              if (
                selectedPath &&
                allFiles.find((f) => f.name === selectedPath) &&
          ...
          {envVars.map(({ key, value }, idx) => (
            <div key={`${key}-${idx}`} className="flex items-center gap-2">
              <EditableSecret
                ...
                conceal={false}
                value={key}
                onSave={(newKey: string) => updateEnvKey(key, newKey)}
              />
              <span className="font-mono text-foreground">=</span>
              <EditableSecret
                className="w-24"
                value={value}
                onSave={(v: string) => updateEnvValue(key, v)}
              />
              <ActionButton
                icon={<Trash2 size={14} />}
                variant="ghost"
