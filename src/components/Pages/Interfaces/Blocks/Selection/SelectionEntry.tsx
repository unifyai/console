...
import { ItemType, LogsActions, TileProps } from '@/types/interfaces/grid';
...
export default function SelectionEntry({
  ...
  const expandRecursively = useMemo(() => {
    return (paths: string[]) =>
      panelSetOpenKeys((prev) => new Set([...Array.from(prev), ...paths]));
  }, [panelSetOpenKeys]);
  ...
  const handleSaveEditForView = useCallback(
    (desc: { logIndex: number; path: (string | number)[]; newValue: any }) => {
      if (onSaveEdit) {
  ...
  const handleGroupSaveEditForView = useCallback(
    (desc: { logIndices: number[]; path: (string | number)[]; newValue: any }) => {
      if (onGroupSaveEdit) {
  ...
  const handleGlobalExpandToggle = (e: React.MouseEvent) => {
    ...
    if (comps && comps.length > 0) {
      currentSubPaths = gatherAllSubPathsMulti(
        rawValue,
        comps,
        topLevelPath,
        source === 'entries' ? 'entries' : 'params',
      ...
      currentSubPaths = gatherAllSubPaths(
        rawValue,
        topLevelPath,
        source === 'entries' ? 'entries' : 'params',
        0
  ...
  const isLeaf = useMemo(
    () => (unifiedType === 'trace' ? false : !isTopLevelExpandable),
    ...
    <AccordionItem value={itemValue}>
      ...
      >
        <div className="inline-flex items-center gap-2">
          ...
          <span
            className={`inline-block align-middle ${cellEditMode && isLeaf ? 'cursor-text' : ''}`}
          >
            {property}
          </span>
          <ActionButton
            ...
            variant="ghost"
            size="sm"
            className="flex items-center justify-center p-0 opacity-0 transition-opacity duration-200 hover:bg-destructive hover:text-destructive-foreground group-hover:opacity-100"
            onClick={(e) => {
              e.stopPropagation();
      ...
      <AccordionContent>
        <div className="relative ml-4 border-l border-l-muted pl-3">{contentNode}</div>
      </AccordionContent>
    </AccordionItem>
  );
}
