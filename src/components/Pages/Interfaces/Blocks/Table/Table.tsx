...
import {
  ...
  encodeRenderedDepth,
  formatCellValue,
} from '@/utils/interfaces/table/table';
...
import RowExpanding, {
  RowExpandingProps,
} from '@/components/Common/Tables/Data/Buttons/RowExpanding';
...
import {
  ...
  useTableGroupedMetricsQuery,
  useTableArgumentsQuery,
...
const LogsTable = ({
  ...
  const groupSortingExpression =
    tableArguments?.[tileName]?.getLogsParameters?.groupSorting || null;
  ...
  useEffect(() => {
    if (
      projectId !== prevProjectIdForOverlayRef.current ||
      item?.context !== prevContextForOverlayRef.current
    ) {
  ...
  const setDefaultHidden = useCallback(
    (val: boolean) => {
  ...
  const dataTypes = useMemo(
    () =>
      fields
        ? Object.fromEntries(
            Object.entries(fields as any).map((entry: any) => [entry[0], entry[1]?.dataType])
          )
        : {},
    [fields]
  );
  const fieldTypes = useMemo(
    () =>
      fields
        ? Object.fromEntries(
            Object.entries(fields as any).map((entry: any) => [entry[0], entry[1]?.fieldType])
          )
        : {},
    [fields]
  );
  const mutabilityMap = useMemo(
    () =>
      fields
        ? Object.fromEntries(
            Object.entries(fields as any).map((entry: any) => [entry[0], entry[1]?.mutable])
          )
        : {},
    [fields]
  );
    ...
    return [
      {
        id: indicesTitle,
        cell: ({ row }) => (
          <Badge>{(row.effectiveIndex !== undefined ? row.effectiveIndex : row.index) + 1}</Badge>
        ),
        meta: {
          dataType: null,
          columnType: 'util',
  ...
  const columnOrder = useMemo(
    () => (columnOrderStr ? columnOrderStr.split(',') : columnIDs),
    [columnOrderStr, columnIDs]
  );
  const setColumnOrder = useCallback(
    (order: string[], manual = true) => {
  ...
  const setColumnVisibility = useCallback(
    (v: { [key: string]: boolean }) => {
      ...
      tableTileActions?.setHiddenColumns(
        hidden.length ? hidden.join(',') : hidden.length === 0 ? '' : undefined
      );
    },
  ...
  const setSorting = useCallback(
    (s: ColumnSort[]) =>
      tableTileActions?.setSorting(
        s.map((item) => `${sanitizeId(item?.id)}@${item?.desc}`).join(',')
      ),
    [tableTileActions]
  );

  const grouping: GroupingState = useMemo(
    () => (groupingStr ? groupingStr.split(',') : []),
    [groupingStr]
  );
  const setGrouping = useCallback(
    (g: GroupingState) => syncedTileDataActions?.setGrouping(g.length ? g.join(',') : undefined),
    [syncedTileDataActions]
  );

  const groupSorting: ColumnSort[] = useMemo(
    () =>
      groupSortingStr
  ...
  const setGroupSorting = useCallback(
    (s: ColumnSort[]) =>
      tableTileActions?.setGroupSorting(
        s.map((item) => `${sanitizeId(item.id)}@${item.desc}`).join(',')
      ),
    [tableTileActions]
  );
  ...
  const calculateGroupHasNextPage = useCallback(
    (groupId: string | undefined): boolean => {
  ...
  const isCellMutable = useCallback(
    (cell: any) => {
    ...
    (cell: any) => {
      try {
        ...
        if (!fieldInfo) {
          showErrorToast(
            'Field not defined',
            'This field is not defined in Logs schema. Define it before editing.'
          );
        } else if (fieldInfo.mutable === false || fieldInfo.mutable === 'false') {
          showErrorToast(
            'Field is immutable',
            'This field cannot be edited. Make it mutable in Logs > Fields or use a versioned context.'
          );
        }
      } catch (_) {}
    },
  ...
  const onCommitCellEdit = useCallback(
    async ({
      rowIds,
      source,
      path,
      ...
      const firstPrev = (tableDataItem?.logs || []).find(
        (l: any) => String(l.id) === String(rowIds[0])
      );
      if (!firstPrev) return;
      ...
      if (updateLogsByRowIds) {
        ...
        try {
          ...
          if (
            (originalAtPath === null || originalAtPath === undefined) &&
      ...
      const isImmutable =
        fieldInfo && (fieldInfo.mutable === false || fieldInfo.mutable === 'false');
      ...
      <PopoverContent className="z-50 w-96 p-0">
        ...
        <div className="flex items-center justify-between border-t border-border p-2">
          ...
          <Button
            ...
            size="sm"
            className="ml-auto"
            onClick={() => setTableContextPopoverOpen(false)}
          >
            Cancel
          </Button>
        </div>
  ...
  const effectiveContextNotFound = useMemo(
    () =>
  ...
  const showActions =
    grouping.length > 0 ||
    sorting.length > 0 ||
    groupSorting.length > 0 ||
    logsFilters != undefined ||
          ...
          <div className="text-center">
            ...
            {infiniteLogsQuery.bidirectionalInfo && showAdvancedFeatures && (
              <div className="text-caption text-muted-foreground">
                Pages: {infiniteLogsQuery.bidirectionalInfo.pagesInMemory}/
                {infiniteLogsQuery.bidirectionalInfo.maxPagesInMemory}
                {infiniteLogsQuery.bidirectionalInfo.windowStart !==
                  infiniteLogsQuery.bidirectionalInfo.windowEnd && (
                  <span>
                    {' '}
                    | Window: {infiniteLogsQuery.bidirectionalInfo.windowStart}-
                    {infiniteLogsQuery.bidirectionalInfo.windowEnd}
                  </span>
                )}
              </div>
  ...
  const renderSectionToggle = (
    ...
    <div
      ...
      onClick={() => toggleSection(setter, visible)}
    >
    ...
    <div className="-mx-1 mb-2">
      <div className="flex flex-row justify-between gap-2">
        <Button
          ...
          className="hover:bg-muted/50 h-6 text-muted-foreground transition-all duration-200 ease-out hover:text-foreground"
        >
          <div
            className={cn(
          ...
          >
            <span className="text-caption text-muted-foreground transition-colors duration-200">
              Table Controls
            </span>
            <div
              className={cn(
      ...
      <div
        className={cn(
          'relative overflow-hidden px-2 transition-all duration-500 ease-out',
          !isMenuCollapsed ? 'max-h-96' : 'pointer-events-none max-h-0'
        )}
      >
        <div
          className={cn(
            'command-scrollbar duration-400 flex flex-nowrap items-stretch overflow-x-auto py-2 transition-all ease-out',
            !isMenuCollapsed
        ...
        >
          ...
          <div
            className={cn(
              'flex flex-col gap-1 border-r pr-4 transition-all duration-300 ease-out',
          ...
          {showActions && projectId && (
            <div
              className={cn(
                'flex flex-col gap-1 border-r pl-4 pr-4 transition-all duration-300 ease-out',
            ...
            >
              ...
              <div className={cn('flex items-center gap-2', !actionsSectionVisible && 'hidden')}>
                ...
                />
                <ResetServerAction
                  condition={sorting.length > 0 || groupSorting.length > 0}
                  type={'sorting'}
                  interactive={interactive}
                  logs={logs}
                  setterFunction={() => {
                ...
                />
                <ResetServerAction
                  condition={logsFilters != undefined || commonFilter != undefined}
                  type={'filters'}
                  interactive={interactive}
                  logs={logs}
                  setterFunction={() => {
                    setLogsFilters({});
                    syncedTileDataActions?.setCommonFilter(undefined);
                  }}
                />
              </div>
            </div>
          ...
          >
            ...
            <div className={cn('flex items-center gap-2', !displaySectionVisible && 'hidden')}>
              ...
              <SettingButton
                tooltip={
                  logs.length === 0
                    ? 'Metrics are unavailable for empty tables'
                    : `${showMetricsRow ? 'Hide' : 'Show'} metrics row`
                }
              ...
              {showAdvancedFeatures && (
                <>
                  ...
                  <SettingButton
                    ...
                    icon={
                      <div className="text-caption text-strong flex h-4 w-4 items-center justify-center">
                        ↕
                      </div>
                    }
                  ...
                  {useBidirectionalLoading && (
                    <div className="flex items-center gap-1">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setBidirectionalConfig((prev) => ({
                            ...prev,
                            maxPagesInMemory: Number(e.target.value),
                          }))
                        }
                        className="text-caption rounded border px-1 py-0.5"
                      >
          ...
          {projectId && (
            <div
              className={cn(
                'flex flex-col gap-1 pl-4 pr-4 transition-all duration-300 ease-out',
                !monitoringSectionVisible && 'max-w-[80px]'
              )}
            >
              {renderSectionToggle(
                setMonitoringSectionVisible,
                monitoringSectionVisible,
                'Monitoring'
              )}
  ...
  const onContainerClick = (event: React.MouseEvent<HTMLElement, MouseEvent>) => {
    if (tableTileActions)
      deselectFromClickOutside(event, containerRef, selectedCells, tableTileActions.setSelected, [
        'LogsTable',
        'LogsTablePreferences',
      ]);
  };
  ...
  useEffect(() => {
    ...
    const pageScrollViewport = pageScrollContainerRef?.current?.querySelector(
      '[data-radix-scroll-area-viewport]'
    );
  ...
  return (
    <div
      ref={containerRef}
      className="flex h-full min-h-0 w-full flex-1 flex-col gap-2 overflow-hidden rounded-md bg-background p-2"
              ...
              <EmptyTableOverlay
                ...
                actionButton={
                  overlayMode === 'contextNotFound'
                    ...
                    : overlayMode === 'context'
                      ? selectContextButton
                      : createLogRedirectButton
                }
                withPulse={overlayMode !== 'contextNotFound'}
                contextName={
                  effectiveContextNotFound
                    ? ((item?.context || tileDataState?.context) ?? undefined)
                    : undefined
                }
              />
            )}
              ...
              {projectId ? (
                <div className="flex h-full gap-2">
                  {Array.from({ length: panelCount }).map((_, idx) => (
                    ...
                    >
                      {error && (
                        <div className="bg-destructive/10 border-destructive/30 absolute right-2 top-2 z-10 flex items-center gap-2 rounded border px-2 py-1 text-destructive">
                      ...
                      <DataTable<LogProps | GroupedLogProps>
                        ...
                        AggregatedCell={(cell, row) => (
                          <AggregatedCell
                            ...
                            metric={state.metric}
                            isGroupLoading={
                              loadingGroups.has(row.id) || loadingGroups.has('_all_groups_')
                            }
                          />
                        )}
                        FooterCell={(
