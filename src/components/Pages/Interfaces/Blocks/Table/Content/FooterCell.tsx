...
const FooterCell = ({
  ...
  const borderThickness =
    column.id === 'RowNumbering' ? 1 : Math.max(1, maxDepth - boundaryDepth + 1);
  ...
  const nestedExpand = (row: Row<any>, expanded: boolean) => {
    ...
    if (row.subRows.length > 0) row.subRows.forEach((r) => nestedExpand(r, expanded));
  };
  ...
  return (
    ...
    >
      <div
        className={`text-label text-strong flex min-h-[1rem] flex-col ${column.id === 'RowNumbering' ? 'items-start justify-start pl-2' : 'items-center justify-center text-ellipsis text-nowrap'}`}
      >
        {children}
      </div>
      {/* Pin handle */}
