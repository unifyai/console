...
const ColumnMetrics = ({
  ...
  const onClick = (metricValue: string) => {
    ...
    try {
      ...
      showSuccessRef.current = () =>
        showSuccessToast(`Metric changed to ${metricValue}.`, undefined, loadingId);
    } catch (error) {
      setLoading(false);
      showSuccessRef.current = null;
    ...
    <BaseDropdown
      ...
      button={
        <ActionButton
          ...
          text={metric}
          icon={
            loading || isMetricsLoading || isFetching ? (
              <LoaderCircle className="animate-spin text-primary" />
            ) : (
              <ChevronDown />
            )
          }
          disabled={!interactive || loading || isMetricsLoading || isFetching}
          variant="ghost"
          size="sm"
    ...
    >
      {metrics.map((metricItem, index) => (
        <DropdownMenuCheckboxItem
          checked={metric === metricItem}
          key={index}
          onClick={() => onClick(metricItem)}
