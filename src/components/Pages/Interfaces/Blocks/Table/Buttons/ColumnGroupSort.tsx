...
const ColumnGroupSort = ({
  ...
  const sortingOrder = isGroupSorted
    ? groupSorting.find((s) => s.id === column.id)!.desc
      ? 'desc'
      : 'asc'
    : false;
  ...
  const menuItem = (
    <DropdownMenuItem
      ...
      className="text-body-sm flex cursor-pointer items-center"
    >
      <div className="-translate-x-1 scale-[0.6]">
        {direction === 'asc' ? (
          <GArrowUp className="translate(-2,0)" />
        ) : (
          <GArrowDown className="translate(-2,0)" />
        )}
      </div>
      {direction === 'asc' ? (
        <span>Sort groups ascending</span>
      ) : (
  ...
  const undoButton = (
    <ActionButton
      ...
      disabled={!interactive || groupSortLoading}
    />
  );
