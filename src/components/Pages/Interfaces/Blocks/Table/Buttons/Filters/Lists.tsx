...
const ListColumnFilter = ({
  ...
  const defaultFilter: ListFilter = useMemo(
  ...
  const append = (
    <BaseDropdown context="tile" button={<ActionButton tooltip="Add new filter" icon={<Plus />} />}>
      {['And', 'Or'].map((method, index) => (
        <DropdownMenuItem
          key={index}
          className="text-body-sm cursor-pointer p-2 hover:bg-primary hover:text-white"
          onClick={() => {
            const newFilters = [...filters];
            newFilters.push({
              key: filters.length,
  ...
  const valueInput = (
    ...
    <Input
      className="text-body-sm -ms-px rounded-s-none shadow-none focus-visible:z-10"
