...
const ImageColumnFilter = ({
  ...
  /* Init filter */
  const initialValue =
    columnFilters[column] && columnFilters[column]['isNone']
      ? columnFilters[column]['isNone']
      : 'None';
  ...
  const states = [
    {
      key: 'None',
      tooltip: `Filter for logs with ${column}`,
      icon: <Filter />,
      variant: undefined,
      next: 'true',
    },
    {
      key: 'true',
      tooltip: `Filter for logs without ${column}`,
      icon: <Circle />,
      variant: 'primary',
      next: 'false',
    },
