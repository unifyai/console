...
const StringColumnFilter = ({ interactive, column, columnFilters, setColumnFilterQuery, open, setOpen, filterLoading, setFilterLoading, setIsFiltered, renderMode, entriesProperties, paramsProperties }: {
    interactive: boolean,
    column: string,
    ...
    const structuredToExpression = (structuredFilters: StringFilter[]): string => {
        ...
        return structuredFilters.map((filter, index) => {
            ...
            if (filter.mode === 'exists') {
                ...
                const quotedValue = filter.value.startsWith('"') && filter.value.endsWith('"') ? filter.value : `"${filter.value}"`;
    ...
    const expressionToStructured = (expr: string): StringFilter[] | null => {
        ...
        while ((match = regex.exec(expr)) !== null) {
            ...
            if (match[3]) { // exists or isNone
                ...
                newFilters.push({ key: key++, mode: match[5].toLowerCase() as 'in' | 'not in', join, value: match[4] });
            }
