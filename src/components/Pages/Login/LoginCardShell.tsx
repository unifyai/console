...
const LoginCardShell = ({ children }: { children: React.ReactNode }) => {
  return (
    <div className="brand-pixel-grid fixed inset-0 bg-background">
      <motion.div
        initial={{ y: '100vh' }}
        animate={{ y: 0 }}
      ...
      >
        <div className="relative flex h-full w-full overflow-y-auto border-border bg-card p-8 shadow-[0_24px_90px_var(--shadow-soft)] md:p-24 xl:h-auto xl:max-h-screen xl:w-[760px] xl:rounded-2xl xl:border">
          {children}
        </div>
      </motion.div>
    </div>
