...
const CodePreview = ({
  ...
  const content =
    selectedVariant === 'package'
      ? code.replace('YOUR_API_KEY', request.headers.Authorization)
      : code;
  ...
  return (
    <ScrollArea className="group/code relative text-sm">
      <div
        className={`absolute right-2 ${selectedVariant === 'package' ? 'top-11' : 'top-2'} z-10 flex items-center rounded-lg bg-background`}
      >
        {copy}
      </div>
      {install}
