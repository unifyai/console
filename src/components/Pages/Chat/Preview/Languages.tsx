...
const Languages = ({ selectedLanguage, setSelectedLanguage, selectedVariant, setSelectedVariant }: {
    ...
    const commonLanguages = [
        ...
        "curl",
        "php",
    ].map((language) => languages.find((l: { key: string }) => l.key === language)!);
    ...
    const otherLanguages = languages
        ...
        .flatMap((language) => language.variants.map((variant) => ({
    ...
    return (
        <div className="flex flex-col h-full gap-1 overflow-auto">
            {commonLanguages.map((language, index) => (
                <CommonLanguage key={index} language={language} setSelectedLanguage={setSelectedLanguage} setSelectedVariant={setSelectedVariant}/>
            ))}
            <OtherLanguages languages={otherLanguages} setSelectedLanguage={setSelectedLanguage} setSelectedVariant={setSelectedVariant}/>
        </div>
    );
};
