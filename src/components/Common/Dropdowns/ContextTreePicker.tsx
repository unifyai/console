...
import { useRenameContextQuery, useDeleteContextQuery } from "@/hooks/Interfaces/Query/useContextsQuery";
	...
	return (
		<div className={className}>
			{(current || inheritedCtx) && (
				<div className="px-2 py-1.5 text-body-sm text-muted-foreground border-b border-border space-y-1">
					{current && (
						<div className="flex items-center gap-2 min-w-0">
							<span className="h-2 w-2 rounded-full bg-primary" />
							<span className="truncate">Current context: <span className="text-foreground" title={current}>{current}</span></span>
							{projectId && contextActions && (
								<DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
									...
									<DropdownMenuContent align="end">
										{(() => {
											const isUnityTasks = (projectId === 'Unity') && (String(current).trim() === 'Tasks');
											return (
												<>
												<DropdownMenuItem onSelect={() => { setNewName(current || ""); setRenameOpen(true); }} disabled={isUnityTasks}>Rename</DropdownMenuItem>
												<DropdownMenuItem onSelect={() => setDeleteOpen(true)} className="text-destructive" disabled={isUnityTasks}>Delete</DropdownMenuItem>
												</>
											);
										})()}
									</DropdownMenuContent>
					...
					{inheritedCtx && (!current || inheritedCtx !== current) && (
						<div className="flex items-center gap-2 min-w-0">
							...
							<span className="truncate">Inherited context: <span className="text-foreground" title={inheritedCtx}>{inheritedCtx}</span></span>
						</div>
					)}
				</div>
			)}
				...
				<Accordion type="multiple" className="w-full">
					{basePrefix && basePrefix.trim() !== "" ? (
						...
						Object.entries(subtree.children)
							...
							.map(([name, child]) => (
								<TreeRow key={child.path} node={child} depth={0} current={current || undefined} onPick={onPick} />
							))
					)}
				</Accordion>
			</div>
				...
				<>
						...
						<BaseDialog
							...
							body={
								<div className="space-y-2 pt-2">
									<Input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="New context name" />
								</div>
							}
							footer={
								<div className="flex gap-2">
									<Button variant="outline" onClick={() => setRenameOpen(false)}>Cancel</Button>
									<Button onClick={() => { renameContext({ projectId, currentName: current as string, newName, actions: contextActions }); setRenameOpen(false); }}>Save</Button>
								</div>
							}
						/>
					)}
					{deleteOpen && (
						<BaseDialog
							...
							body={<div className="pt-2">Are you sure you want to delete &quot;{current}&quot;?</div>}
							footer={
								<div className="flex gap-2">
									<Button variant="outline" onClick={() => setDeleteOpen(false)}>Cancel</Button>
									<Button variant="destructive" onClick={() => { deleteContext({ projectId, contextName: current as string, actions: contextActions }); setDeleteOpen(false); onPick(""); }}>Delete</Button>
								</div>
							}
						/>
					)}
...
function TreeRow({ node, depth, current, onPick }: { node: TreeNode; depth: number; current?: string; onPick: (ctx: string) => void }) {
					...
					<div className="relative">
						...
						<ul className="mt-1 space-y-1">
							{entries
								.sort((a, b) => a[0].localeCompare(b[0]))
								.map(([childName, child]) => (
									<TreeRow key={child.path} node={child} depth={depth + 1} current={current} onPick={onPick} />
								))}
						</ul>
					</div>
				</AccordionContent>
