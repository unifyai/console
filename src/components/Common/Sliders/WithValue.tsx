...
const SliderWithValue = ({
  ...
  const onChangeInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(parsedValue)) {
      // Only proceed if it's a valid number (or becomes valid)
  ...
  return (
    <div className="flex flex-col gap-2 p-2 pb-4">
      ...
      <div className="flex w-full grow flex-col px-2">
        <span
          className="text-caption text-strong mb-2 flex w-full items-center justify-between gap-2 text-muted-foreground"
          aria-hidden="true"
        >
          <span>{minimum}</span>
          <span>{maximum}</span>
