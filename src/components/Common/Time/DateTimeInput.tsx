...
export type Attributes = {
  [key in TimeType]: { symbol: string; padding: number; min: number; max: number; format: RegExp };
};
...
const getAttributes = (relative: boolean): Attributes => {
  ...
  const [regExMonth, regExDay] = relative
    ? [/^(0[0-9]|1[0-2])$/, /^(0[0-9]|1[0-9]|2[0-9]|3[0-1])$/]
...
export function setRelativeDateValue(
    ...
    */
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (['ArrowUp', 'ArrowDown'].includes(e.key)) {
        ...
        const newValue = relative
          ? String(newOffset)
          : getIncrement(calculatedValue, picker, step, attributes);
        const newDate = relative
          ? (setRelativeDateValue(
              date as RelativeDateString,
              newValue,
              picker,
              attributes
            ) as RelativeDateString)
          : (setDateValue(
              date as AbsoluteDateString,
              newValue,
              picker,
              attributes
            ) as AbsoluteDateString);
        setOffset(newOffset);
        setDate(newDate);
      }
    ...
    return (
      <div className="group relative items-center">
        {relative && (
          <span className="text-caption text-strong pointer-events-none absolute inset-y-0 start-1.5 flex items-center pe-3 group-focus-within:text-accent-foreground">
            -
          </span>
        )}
        ...
        <Input
          ...
          name={name || picker}
          className={cn(
            'text-body w-[54px] rounded-none text-start font-mono tabular-nums caret-transparent focus:bg-accent focus:text-accent-foreground [&::-webkit-inner-spin-button]:appearance-none',
            className
          )}
        ...
        <span className="text-caption pointer-events-none absolute inset-y-0 end-0 flex items-center pe-3 text-muted-foreground group-focus-within:text-accent-foreground">
          {attributes[picker].symbol}
        </span>
      </div>
    );
  }
