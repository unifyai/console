...
const DeleteDialog = ({
  ...
  const button = setShowDialog ? null : (
    <ActionButton
      tooltip={tooltip}
      icon={icon}
      text={text}
  ...
  const body = (
    <div className="text-body">
      {success ? messages['success'] : error ? messages['error'] : messages['warning']}
    </div>
  );
        ...
        <ActionButton
          ...
          onClick={() => {
            ...
            const uniqueIds = Array.from(
              new Set((idsAndFieldsArg as [number, any][]).map(([id]) => id))
            );
  ...
  return (
    <BaseDialog
      ...
      setOpen={setShowDialog ? setShowDialog : setOpen}
    />
  );
};
