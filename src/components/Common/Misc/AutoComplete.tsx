    ...
    <Popover open={effectiveOpen} onOpenChange={handleOpenChange}>
      <PopoverTrigger asChild>
        <Button
          ...
          className={cn(
            `bg-background/90 border-border/50 h-7 border shadow-md backdrop-blur-sm transition-all duration-200 hover:bg-accent hover:text-accent-foreground`,
            displayMode === 'icon' ? 'w-7 justify-center px-2' : 'w-[200px] justify-between px-2',
            className
          )}
          disabled={disabled}
        >
        ...
        {loading ? (
          ...
          <Command>
            <CommandInput
              ...
              className="border-none bg-transparent [&_[cmdk-input-wrapper]]:border-none"
              disabled={disabled}
            />
            <CommandList>
              ...
              <CommandGroup>
                {items.map((item) => (
                  <CommandItem
                    ...
                    className="text-body-sm h-8 cursor-pointer"
                  >
