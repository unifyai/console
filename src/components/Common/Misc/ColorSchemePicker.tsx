    ...
    <div className="grid gap-2">
      {colorSchemes.map((scheme) => (
        <button
          ...
          className={cn(
            'text-body flex w-full cursor-pointer items-center justify-between rounded-md p-2 text-left hover:bg-accent',
            value === scheme.name && 'text-strong bg-accent',
            rowClassName
          )}
        ...
        >
          ...
          <div
            className={cn('flex flex-wrap justify-end space-x-1 overflow-hidden', paletteClassName)}
          >
            {scheme.palette.map((color, index) => (
              <div
                key={`${scheme.name}-swatch-${index}`}
                className={cn('h-4 w-4 flex-shrink-0 rounded-sm', swatchClassName)}
  ...
  if (useDialog) {
    return (
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="max-h-[30rem] w-[25rem] overflow-auto">
          <div className="pb-4">
