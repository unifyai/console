...
const ColumnSort = ({
  ...
  const icon = sortLoading ? (
    <LoaderCircle
      className={`animate-spin ${isSorted ? 'text-primary-foreground' : 'text-primary'}`}
    />
  ) : (
    state.icon
  );
  ...
  const onClick = () => {
    ...
    setSortLoading(true);
    if (!column.getNextSortingOrder()) setSpinnerColor('primary');
    else setSpinnerColor('primary-foreground');
  };

  return renderMode === 'menuItem' ? (
    <DropdownMenuItem onClick={onClick} className="flex cursor-pointer items-center gap-2">
