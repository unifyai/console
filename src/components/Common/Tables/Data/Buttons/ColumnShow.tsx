...
import {
  getImmediateRightNeighbors,
  getImmediateHiddenSiblings,
...
const ColumnShow = ({
  ...
  const rawHiddenColumns = (() => {
    ...
    if (
      ...
      isLastSameParentColumnInColumnOrder(header.column, columnOrder, columnVisibility)
    ) {
      ...
      const hiddenImmediateRightNeighbors = immediateRightNeighbors.filter(
        (neighbor) => !columnVisibility[neighbor]
      );
  ...
  const derived =
    ColumnCreate && (columnType === 'entries' || columnType === 'util')
      ? ColumnCreate(header.column.id, setOpen)
      : null;

  return (
    <div className="absolute -right-4 z-50 scale-[0.85] opacity-0 transition-all hover:opacity-100">
