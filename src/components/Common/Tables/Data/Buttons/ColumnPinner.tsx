...
const ColumnPinner = ({
  ...
  const handleDragMove = useCallback(
    (e: MouseEvent) => {
      ...
      setOverlayPosition((prev) => (prev ? { ...prev, left: overlayStartLeft + dragDelta } : null));
  ...
  const commitPinChanges = useCallback(
    (finalLeft: number) => {
      const leafs: Column<any, unknown>[] =
        (table as any).getVisibleLeafColumns?.() ?? (table as any).getLeafColumns?.() ?? [];
  ...
  useEffect(() => {
    ...
    setOverlayPosition((prev) =>
      prev ? { ...prev, top: metrics.top, height: metrics.height } : prev
    );
