...
interface DataTableProps<TData extends LogProps | GroupedLogProps> {
  ...
  FooterCell?: (
    column: TanstackColumn<any | unknown>,
    resizeMap: { [x: string]: (event: unknown) => void },
    table: TanstackTable<any | unknown>,
    draggingColumnPinner: DraggingColumnPinnerState,
    setDraggingColumnPinner: (draggingColumnPinner: DraggingColumnPinnerState) => void,
  ...
  ExtraCellContent?: (
    ...
    isCellExpanded: (cell: TanstackCell<any, unknown>) => boolean,
...
export default function DataTable<TData extends LogProps | GroupedLogProps>({
  ...
  const setUpdatedState = (state: any, setterFunction: (x: any) => void, updater: Updater<any>) => {
  ...
  const [columnActionsApplied, setColumnActionsApplied] = useState<{
    [depth: number]: { [columnId: string]: boolean };
  }>({});
  ...
  const table = useReactTable({
    ...
    onColumnVisibilityChange: (updater: Updater<{ [k: string]: boolean }>) =>
      setUpdatedState(state.columnVisibility, setState.setColumnVisibility, updater),
    onColumnOrderChange: (updater: Updater<string[]>) =>
      setUpdatedState(state.columnOrder, setState.setColumnOrder, updater),
    ...
    onColumnFiltersChange: (updater: Updater<ColumnFiltersState>) =>
      setUpdatedState(state.columnFilters, setState.setColumnFilters, updater),
    onColumnPinningChange: (updater: Updater<ColumnPinningState>) =>
      setUpdatedState(state.columnPinning, setState.setColumnPinning, updater),
  ...
  useEffect(() => {
    ...
    const newOrder = [...(rowNum ? [rowNum] : []), ...grouped, ...remaining];
    if (JSON.stringify(newOrder) !== JSON.stringify(currentOrder)) {
      setState.setColumnOrder(newOrder);
    }
  }, [state.columnOrder, state.grouping, setState]);
  ...
  const resizeMap = table
    .getFlatHeaders()
    .map((header: Header<TData, unknown>) => ({ [header.column.id]: header.getResizeHandler() }))
    .reduce(
      (
        acc: { [key: string]: (event: unknown) => void },
        curr: { [key: string]: (event: unknown) => void }
      ) => ({ ...acc, ...curr }),
      {}
    );

  const { isCellSelected, isRowSelected, isCellExpanded, setExpandedCells, ...cellSelection } =
    useCellSelection({
      table,
  ...
  const VirtualRow = ({ index, style }: ListChildComponentProps) => {
    ...
    return <div style={style}>{renderHierarchicalRow(topLevelRow, subRows)}</div>;
  };
  ...
  return (
    <div className="relative flex h-fit w-full min-w-max gap-2">
      <DndContext
        sensors={sensors}
      ...
      >
        ...
        >
          ...
          <TableBody
            className="contents overflow-y-auto"
            ...
            {isUpdatingLogs ? (
              ...
              <TableRow>
                <TableCell
                  ...
                  className="w-full min-w-[150px] text-wrap text-start text-warning"
                  style={{
              ...
              enableVirtualization ? (
                ...
                <>
                  ...
                  {(() => {
                    const { topLevelRows, subRows } = processRowsHierarchy(
                      table.getRowModel().rows
                    );

                    return topLevelRows.map((row) => renderHierarchicalRow(row, subRows));
                  })()}
          ...
          <TableFooter ref={tableFooterRef} className="sticky bottom-0 z-20 bg-background pt-2">
            ...
            {table.getRowModel().rows?.length > 0 && (
              <TableRow className="relative">
                <TableCell
                  colSpan={finalColumns.length}
                  className="group/footer-resizer-cell relative h-1 border-t-0 p-0"
                >
                  ...
                  >
                    ...
                    {FooterCell &&
                      FooterCell(
                        ...
                        resizeMap,
                        table,
                        state.draggingColumnPinner,
                        setState.setDraggingColumnPinner,
                        state.columnPinning,
                        state.columnOrder,
