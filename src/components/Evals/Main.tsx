...
const Main = async ({ searchParams, projectsActions, logsActions, fieldsActions, derivedEntryActions }: {
	...
	let logsData: LogsResponseProps = { params: {}, logs: [], count: 0, groups: {} };
	const limit = 20;
	const offset = (searchParams.page_number ? parseInt(searchParams.page_number) : 0) * limit;
	let totalPages = 1;
	let plotData: LogsResponseProps = { params: {}, logs: [], count: 0, groups: {} };
	...
	if (project) {
		...
		logsData = await logsActions.get(project, context ?? null, null, filterExpression, sortingExpression, groupingExpression, null, null, null, null, limit, offset, groupingExpression ? 0 : null, "False", "False", _timestamp)
		...
		if (xAxis) {
			...
			if (searchParams.plot_type === "Bar Chart") 
				plotData = await logsActions.get(project, context ?? null, null, filterExpression, null, null, null, null, subset, null, null, 0, null, null, "False", _timestamp)
			else {
				if (yAxis)
					subset += `%26${yAxis}`
