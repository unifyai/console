...
const SearchFilter = ({
    ...
    const handleChipClose = (query: string) => {
        ...
        const newQuery = searchQuery.split(" ").filter((q) => q != query).join(" ");
        ...
        <div className="flex flex-row gap-1 flex-wrap">
            ...
            {queryChips.map((query, index) => 
                <Chip
                    ...
                    button={
                        <ActionButton 
                            icon={<p className="text-gray-500 hover:text-foreground mb-1">x</p>}
    ...
    const EndContent = searchQuery && <Chip button={
            <ActionButton 
                icon={<p className="text-gray-500 hover:text-foreground mb-1">x</p>}
