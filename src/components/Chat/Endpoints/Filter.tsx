...
const ProviderFilter = ({endpoints, excludedProviders, setProviderFilterParam}: {
    endpoints: Endpoint[],
    ...
    const onSelection = (value: CheckedState, provider: string) => {
        if (!value) setProviderFilterParam([...excludedProviders, provider].join(","));
            ...
            setProviderFilterParam(newExcludedProviders.length > 0 ? newExcludedProviders.join(",") : null);
        }
    }
    ...
    return (
        <BasePopover
            button={
                <ActionButton icon={<Filter/>} tooltip="Filter providers"/>
            }
        >
            <div className="flex items-center space-x-2 gap-2">
                <Checkbox 
                    id={"All providers"} 
                    checked={allChecked} 
                    onCheckedChange={() => !allChecked ? setProviderFilterParam(null) : setProviderFilterParam(providers.join(","))} 
