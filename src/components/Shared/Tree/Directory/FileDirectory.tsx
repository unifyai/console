...
export default function FileDirectory({
  ...
  const handleSelection = useCallback(
    (selection: string | undefined, files: NodeProps[]) => {
  ...
  const onSubmit = async () => {
    ...
    const mapping = newFiles.map((file) => ({
      oldName: oldFiles.find((f) => f.name === file.name)!.path,
      newName: file.path,
    }));
  ...
  return (
    <div className="flex flex-row items-center gap-2">
      <BaseDialog
        ...
        disabled={disabled}
      />
      {type != 'Projects' && !hideAutocomplete && (
        <AutoComplete
          type={type}
