...
function renderMenuItem(
  ...
  return (
    <SidebarMenuItem
      key={item.title}
      className={`px-3 py-1 transition-colors ${isSubItem ? 'group-data-[collapsible=icon]:px-1.5 group-data-[collapsible=icon]:py-0.5' : ''}`}
    >
        ...
        <Link
          ...
          className={`flex items-center rounded-sm transition-colors ${
            isActive
              ...
              : 'hover:bg-transparent hover:text-sidebar-primary'
          }`}
        >
          <TooltipProvider>
            <Tooltip>
          ...
          <span className="ml-2 select-none group-data-[collapsible=icon]:hidden">
            {item.title}
          </span>
        </Link>
      </SidebarMenuButton>
    ...
    <SidebarMenuItem className="px-2 py-1 transition-colors">
      <div className="flex w-full items-center">
        <SidebarMenuButton
          asChild
          isActive={isActive}
          className={`flex-grow items-center rounded-sm transition-colors ${
            isActive
              ? 'bg-primary font-bold text-primary-foreground'
              : 'hover:bg-transparent hover:text-sidebar-primary'
          }`}
        >
          <Link href={item.href} className="flex items-center">
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <ItemIcon className={`h-5 w-5 ${isActive ? 'text-primary-foreground' : ''}`} />
                </TooltipTrigger>
                <TooltipContent side="right">
                  <p>{item.title}</p>
                </TooltipContent>
        ...
        <button
          ...
          className="ml-1 rounded-sm p-1 hover:bg-muted focus:outline-none focus:ring-1 focus:ring-ring group-data-[collapsible=icon]:hidden"
...
export default function NavMenu() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const favourites = (await res.json()) as {
          id: number;
          projectName: string;
          icon: string;
          position: number;
  ...
  const SortableProjectItem = ({
    ...
    item: ProjectItem;
    onUnfavouriteClick: (favId: number, title: string) => void;
  }) => {
    ...
    const projectNavItem: NavItem = {
      title,
      icon: () => <Icon name={iconName as any} className="h-5 w-5" />,
    ...
    return (
      <SidebarMenuItem
        ...
        className={`group flex select-none items-center justify-between bg-transparent px-2 py-1 ${isDragging ? '!bg-transparent opacity-50 [&_*]:!bg-transparent' : ''}`}
      >
        <div className="min-w-0 flex-grow">
          {' '}
          {/* min-w-0 for proper truncation with flex */}
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <SidebarMenuButton
                  asChild
                  className={`w-full cursor-grab bg-transparent hover:bg-transparent ${isProjectActive ? 'bg-primary data-[active=true]:bg-primary' : 'data-[active=true]:bg-transparent'} ${isDragging ? '!bg-transparent [&_*]:!bg-transparent' : 'hover:text-sidebar-primary'}`}
                  style={{
                ...
                >
                  <Link
                    href={`/interfaces?project=${encodeURIComponent(title)}`}
                    className={`flex w-full items-center rounded-sm ${isProjectActive ? 'bg-primary font-bold text-primary-foreground' : 'bg-transparent'}`}
                    style={{
                      background: isProjectActive ? 'var(--primary)' : 'transparent',
                      backgroundColor: isProjectActive ? 'var(--primary)' : 'transparent',
                    }}
                  >
                    ...
                    <span
                      className={`ml-2 select-none truncate group-data-[collapsible=icon]:hidden ${
                        isDragging
                          ? 'bg-transparent'
                          : isProjectActive
                            ? 'text-primary-foreground'
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                className="ml-1 flex-shrink-0 p-1 text-muted-foreground hover:text-primary focus:outline-none group-data-[collapsible=icon]:hidden"
            ...
            <TooltipContent side="right">
              <p>{`Remove ${item.title} from favourites`}</p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      </SidebarMenuItem>
  ...
  return (
    <Sidebar collapsible="icon">
      ...
      <SidebarContent className="mt-2 flex h-full list-none flex-col">
                        ...
                        >
                          {projects.map((p) => (
                            <SortableProjectItem
                              key={p.id}
                              item={p}
                              onUnfavouriteClick={handleUnfavourite}
                            />
                          ))}
                        </SortableContext>
                      </DndContext>
      ...
      <SidebarFooter className="flex flex-row items-center gap-2 px-3 py-2.5">
