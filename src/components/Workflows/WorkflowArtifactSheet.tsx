...
/**
 * One bundled artifact, previewed in place — a nested sheet stacked above
 * the workflow drawer, so reading a procedure or a task brief never means
 * leaving the install flow. Back returns to the drawer exactly as it was;
 * "Open in <tab>" remains for the reader who wants the live object's page.
 *
 * The body is the published curated copy from the Builtins shelf. After an
 * install the planted row is the live copy and may diverge (the user owns
 * it); its page — not this preview — is where edits happen and show.
 */
export function WorkflowArtifactSheet({
  artifact,
  open,
  isLoading,
  onBack,
  ...
  return (
    <Sheet open={open} onOpenChange={(next) => !next && onBack()}>
      <SheetContent
        side="right"
        className="flex !w-[min(560px,calc(100vw-2rem))] !max-w-[calc(100vw-2rem)] flex-col gap-0 overflow-hidden p-0 sm:!max-w-[calc(100vw-2rem)]"
      ...
      >
        <header className="flex items-center gap-2 border-b p-3.5">
          ...
          <div className="min-w-0 flex-1">
            ...
            {surface && (
              <SheetDescription className="text-caption mt-0.5">
