 ...
 */
export function useTileItem(tileIdOrName: string | null, tabIdOrName: string | null) {
  ...
  const { tableTile } = useTableTile(tileIdOrName, tabIdOrName || null);

  const { plotTile } = usePlotTile(tileIdOrName, tabIdOrName || null);

  const { viewTile } = useViewTile(tileIdOrName, tabIdOrName || null);

  const { editorTile } = useEditorTile(tileIdOrName, tabIdOrName || null);

  const { terminalTile } = useTerminalTile(tileIdOrName, tabIdOrName || null);
  ...
  const itemActions = useMemo(() => {
    return createTileItemActions(tileId || '', tile, storeUpdateTile);
  }, [tileId, tile, storeUpdateTile]);
...
export function useTileItemActions() {
  ...
  const getTileItemActions = useCallback(
    (tileIdOrName: string, tabIdOrName: string | null) => {
      ...
      return createTileItemActions(tileLookup.id, tileLookup, state.updateTile);
    },
    [storeApi]
  );
