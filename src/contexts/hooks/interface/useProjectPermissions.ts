...
import { ResourceAccessListResponse, ResourceAccessResponse } from '@/types/resource';
...
export function useProjectPermissions(
...
): ProjectPermissions & { refetch: () => void } {
  ...
  const permissions = useMemo((): Omit<ProjectPermissions, 'refetch'> => {
    ...
    if (
      ...
      (isOrgProject && (accessQuery.isLoading || rolesQuery.isLoading))
    ) {
      return {
        hasWrite: false,
        hasDelete: false,
    ...
    if (projectQuery.isError) {
      return {
        ...
        isLoading: false,
        isError: true,
        error: (projectQuery.error as Error).message,
        accessEntries: [],
        availableRoles: [],
        availableTeams: [],
        availableMembers: [],
