...
import {
  ...
  useCreateOnlyProjectQuery,
} from '@/hooks/Interfaces/Query/useProjectsQuery';
import {
  ProjectsActions,
...
export function useCommand(args: UseCommandArgs) {
      ...
      if (newProj && setInterfaceQueryParam) {
        try {
          ...
          if (interfacesList && interfacesList.length > 0) {
            ...
            debugLog(
              '[selectProject] No interfaces found, navigating to interface selector for manual creation'
            );
            setDemo(null);
            setTabQueryParam(null);
        ...
        try {
          ...
          const hasEnv = Array.isArray(res)
            ...
            : Array.isArray(res.files) &&
              res.files.some((e: any) =>
                typeof e === 'string' ? e === '.env' : e.name === '.env'
              );
          if (!hasEnv) {
          ...
          debugLog(
            '[selectProject] Project directory may not exist, creating .env to initialize:',
            newProj
          );
          try {
            await fileActions.write(newProj, { '.env': '' });
        ...
        const content =
          res && typeof res === 'object' && 'content' in res ? (res as any).content : '';
        if (content === '') {
        ...
        debugLog(
          '[selectProject] No project selected or missing setInterfaceQueryParam, using demo mode'
        );
        setDemo(null);
  ...
  const createProject = useCallback(
    async (name: string): Promise<ResponseProps> => {
  ...
  const deleteProjectLogs = useCallback(
    async (name: string): Promise<ResponseProps> => {
      if (!logsActions)
        return { error: 'Logs actions not configured.' } as unknown as ResponseProps;
  ...
  const deleteProjectLogsAndContexts = useCallback(
    async (name: string): Promise<ResponseProps> => {
      if (!logsActions || !contextActions)
        return { error: 'Logs or Context actions not configured.' } as unknown as ResponseProps;

      debugLog(
        '[deleteProjectLogsAndContexts] Starting logs and contexts deletion for project:',
        name
      );
      try {
        // 1. Delete all logs
  ...
  const resetTabCommand = useCallback(async () => {
    ...
    debugLog(
      '[resetTabCommand] Starting tab reset for project:',
      projectId,
      'interface:',
      interfaceId
    );
  ...
  const commands = useMemo<Command[]>(() => {
    ...
    debugLog(
      '[commands] Built command list with',
      commandList.length,
  ...
  useEffect(() => {
    if (
      commands.length > 0 &&
      (!storeCommands.length || storeCommands.length !== commands.length)
    ) {
      debugLog(
        '[useEffect] Updating store commands. Previous count:',
        storeCommands.length,
        'New count:',
        commands.length
      );
