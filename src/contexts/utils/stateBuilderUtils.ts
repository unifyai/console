...
import {
  buildInterfaceState as buildInterfaceObject,
  addTabToInterface,
} from './builders/interfaceStateBuilder';
import {
  buildTabState as buildTabObject,
  addTileToTab,
  updateTabParentReferences,
} from './builders/tabStateBuilder';
import {
  buildTileState as buildTileObject,
  updateTileParentReferences,
} from './builders/tileStateBuilder';
...
export function buildProjectStateForStore(
  ...
  if (Array.isArray(projectId)) {
    ...
    for (let i = 0; i < projectId.length; i++) {
      ...
      const ctxs = Array.isArray(contexts[0])
        ? (contexts[i] as Context[])
        : (contexts as Context[]);
      const ids = Array.isArray(interfaceIds[0])
        ? (interfaceIds[i] as string[])
...
export function buildInterfaceStateForStore(
  ...
  if (Array.isArray(interfaceData)) {
    ...
    for (let i = 0; i < interfaceData.length; i++) {
      ...
      const tNames = Array.isArray(tabNames![0])
        ? (tabNames![i] as string[])
        : (tabNames as string[]);
    ...
    if (
      ...
      interfaceData[0].id &&
      result.interfacesById![String(interfaceData[0].id)]
    ) {
  ...
  // Build interface object
  const builtInterface = buildInterfaceObject(
    interfaceData,
...
export function buildTabStateForStore(
  ...
  if (Array.isArray(tabData)) {
    ...
    for (let i = 0; i < tabData.length; i++) {
      ...
      const tNames = Array.isArray(tileNames![0])
        ? (tileNames![i] as string[])
        : (tileNames as string[]);
      ...
      const updatedTab = iId ? updateTabParentReferences(tab, iId) : tab;
  ...
  // Update parent references if interfaceId is provided
  const updatedTab = interfaceId ? updateTabParentReferences(tab, interfaceId as string) : tab;
...
export function buildTileStateForStore(
  ...
  if (Array.isArray(tileData)) {
    ...
    for (let i = 0; i < tileData.length; i++) {
      ...
      const updatedTile = updateTileParentReferences(tile, tId || null);
  ...
  // Update parent references if provided
  const updatedTile = updateTileParentReferences(tile, (tabId as string) || null);
