...
export interface ContextsActions {
  ...
  setContextOptimistic: (
    scope: ContextScope,
    targetId: string,
    context: string,
    meta?: ContextSyncJobMeta
  ) => void;
  enqueueContextSync: (
    scope: ContextScope,
    targetId: string,
...
> = (set, get) => ({
  ...
  enqueueContextSync: (scope, targetId, context, meta) =>
    set((state) => {
      // Replace any existing job for same scope+target with the latest payload
      const existingIdx = state.contextSyncQueue.findIndex(
        (j) => j.scope === scope && j.targetId === targetId
      );
  ...
  dequeueContextSync: () => {
    ...
    set((s) => {
      s.contextSyncQueue = s.contextSyncQueue.slice(1);
    });
    return job;
  },
  ...
  setProcessingQueue: (processing) =>
    set((s) => {
      s.processingQueue = processing;
    }),

  clearInvalidContexts: (projectId) =>
    set((state) => {
  ...
  renameProjectContext: (projectId, from, to) =>
    set((state) => {
      if (!from || from === to) return;
      // Update context list
      const list = state.projectContexts[projectId] || [];
      state.projectContexts[projectId] = Array.from(
        new Set(list.map((n) => (n === from ? to : n)))
      );
  ...
  deleteProjectContext: (projectId, name) =>
    set((state) => {
      if (!name) return;
      // Remove from context list
      state.projectContexts[projectId] = (state.projectContexts[projectId] || []).filter(
        (n) => n !== name
      );
      // Update interfaces in the project
      const ifaceIds: string[] = (state.projectsById?.[projectId]?.interfaceIds || []) as string[];
