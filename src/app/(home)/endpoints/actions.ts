...
export const listCustomEndpoints = async (apiKey: string) => {
  return async (): Promise<CustomEndpoint[]> => {
    ...
    const response = await fetch(`${process.env.NEXTAUTH_URL}/api/customEndpoints/list`, {
      method: 'GET',
...
export const createCustomEndpoint = async (apiKey: string) => {
  return async (
    ...
    const response = await fetch(
      `${process.env.NEXTAUTH_URL}/api/customEndpoints?name=${name}&url=${url}&keyName=${keyName}` +
        (modelArg ? `&modelArg=${modelArg}` : ''),
...
export const deleteCustomEndpoint = async (apiKey: string) => {
  return async (name: string): Promise<ResponseProps> => {
    ...
    const response = await fetch(`${process.env.NEXTAUTH_URL}/api/customEndpoints?name=${name}`, {
      method: 'DELETE',
      headers: { apiKey: apiKey },
    });
    return await response.json();
