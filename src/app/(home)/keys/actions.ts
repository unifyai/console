...
export const listProviders = (apiKey: string) => {
  return async (): Promise<string[]> => {
    ...
    const response = await fetch(`${process.env.NEXTAUTH_URL}/api/providers`, {
      method: 'GET',
...
export const listCustomKeys = async (apiKey: string) => {
  return async (): Promise<CustomKey[]> => {
    ...
    const response = await fetch(`${process.env.NEXTAUTH_URL}/api/customKeys/list`, {
      method: 'GET',
      headers: { apiKey: apiKey },
    });
    return await response.json();
...
export const deleteCustomKey = async (apiKey: string) => {
  return async (name: string): Promise<ResponseProps> => {
    ...
    const response = await fetch(`${process.env.NEXTAUTH_URL}/api/customKeys?name=${name}`, {
      method: 'DELETE',
      headers: { apiKey: apiKey },
    });
    return await response.json();
