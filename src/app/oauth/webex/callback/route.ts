 ...
 *
 * Handles the redirect Webex sends after the user grants consent.
 ...
 * writes ``WEBEX_REFRESH_TOKEN`` back to the assistant.
 *
 * Mirrors ``src/app/oauth/employmenthero/callback/route.ts`` minus the
 * organisations enumeration step — Webex tokens are already org-scoped,
 * so there is no active-org pinning to do here.
 */
...
export async function GET(request: NextRequest) {
  ...
  // Webex-side error short-circuit: redirect with the error code.  We
  ...
  let tokens;
  try {
    tokens = await exchangeWebexCode({
      clientId,
      clientSecret,
      code,
      redirectUri,
  ...
  //    any real auth issue on first use.  We just won't have a name to
  //    show in the success toast.
  try {
    await fetchWebexAccountInfo({ accessToken: tokens.access_token });
  } catch {
    // Swallow: token may lack spark:people_read; that's OK for some
    // workflows.
  }
  ...
  const writes: Array<{ name: string; value: string }> = [
    { name: 'WEBEX_REFRESH_TOKEN', value: tokens.refresh_token },
  ];
