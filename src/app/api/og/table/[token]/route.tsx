...
const colors = {
  ...
  // Table colors
  tableHeader: '#4cc552',
  tableHeaderBg: 'rgba(76, 197, 82, 0.08)',
  tableCellText: '#f5f5f5',
  tableBorder: 'rgba(47, 79, 79, 0.5)',
...
function formatValue(value: unknown): string {
  ...
  if (typeof value === 'string' && value.length > 25) return value.slice(0, 25) + '...';
  return String(value);
}
...
export async function GET(request: NextRequest, { params }: { params: { token: string } }) {
  ...
  const fieldDataCounts: Record<string, number> = {};
  allFields.forEach((field) => {
    fieldDataCounts[field] = 0;
    previewRows.forEach((row) => {
      const val = row[`table1.${field}`] ?? row[field];
      if (val !== null && val !== undefined && val !== '') {
        fieldDataCounts[field]++;
      }
    });
  });

  // Sort fields by data count (most data first), then alphabetically
  const sortedFields = [...allFields].sort((a, b) => {
    const countDiff = fieldDataCounts[b] - fieldDataCounts[a];
    if (countDiff !== 0) return countDiff;
    return a.localeCompare(b);
  });

  const previewColumns = sortedFields.slice(0, 5);
    ...
    >
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          >
            ...
            <span style={{ marginLeft: 16 }}>{rowCount.toLocaleString()} rows</span>
            <span style={{ color: colors.muted, marginLeft: 16 }}>•</span>
            <span style={{ marginLeft: 16 }}>{columnCount} columns</span>
          </div>
        </div>
        <div
          style={{
          ...
          {previewColumns.map((col, i) => (
            <div
              ...
              style={{
                ...
                fontWeight: 600,
                borderRight:
                  i < previewColumns.length - 1 ? `1px solid ${colors.tableBorder}` : 'none',
                overflow: 'hidden',
                textOverflow: 'ellipsis',
                whiteSpace: 'nowrap',
              }}
        ...
        {previewRows.map((row, rowIdx) => (
          <div
            ...
            style={{
              display: 'flex',
              borderBottom:
                rowIdx < previewRows.length - 1 ? `1px solid ${colors.tableBorder}` : 'none',
              background: rowIdx % 2 === 1 ? colors.tableRowAlt : 'transparent',
            }}
          >
            {previewColumns.map((col, colIdx) => (
              <div
                ...
                style={{
                  ...
                  fontSize: 15,
                  borderRight:
                    colIdx < previewColumns.length - 1 ? `1px solid ${colors.tableBorder}` : 'none',
                  overflow: 'hidden',
                  textOverflow: 'ellipsis',
                  whiteSpace: 'nowrap',
                }}
              >
                {formatValue(row[`table1.${col}`] ?? row[col])}
              </div>
            ))}
            {allFields.length > 5 && (
              <div
        ...
        {rowCount > 4 && (
          <div
            style={{
              ...
              fontSize: 14,
              background: `linear-gradient(to bottom, transparent, ${colors.eerieBlack})`,
            }}
          >
            + {(rowCount - 4).toLocaleString()} more rows
          </div>
        )}
      </div>
      ...
      >
        <div style={{ color: colors.muted, fontSize: 16 }}>
          Click to view full interactive table
        </div>
