...
export async function POST(request: NextRequest) {
  ...
  const results: any[] = [];
  const errors: Array<{ id?: string; tabId?: string; name?: string; error: string }> = [];
  ...
  for (const chunk of chunks) {
    const responses = await Promise.allSettled(
      chunk.map(async (u) => {
        // Build URL based on id or (tabId + name)
        // Transform query param keys to snake_case for Orchestra
        let url = '';
        if (u.id) {
          url = `${baseUrl}/tile/?tile_id=${encodeURIComponent(u.id)}`;
        } else if (u.tabId && u.name) {
          const qs = new URLSearchParams();
          qs.set(camelToSnake('tabId'), u.tabId);
          qs.set('name', u.name);
          url = `${baseUrl}/tile/?${qs.toString()}`;
        } else {
          throw new Error('Missing id or (tabId + name)');
        }
        ...
        try {
          const res = await fetch(url, {
            ...
            body: JSON.stringify(camelToSnakeObject(u.updateData || {})),
          ...
          return { ok: true, ref: { id: u.id, tabId: u.tabId, name: u.name }, data: json };
        } catch (e: any) {
          clearTimeout(ttl);
    ...
    responses.forEach((r, idx) => {
      ...
      if (r.status === 'fulfilled' && (r.value as any)?.ok) {
        ...
        errors.push({
          ...
          tabId: ref.tabId,
          name: ref.name,
          error: (r as any)?.reason?.message || 'Failed',
        });
      }
    });
  }
