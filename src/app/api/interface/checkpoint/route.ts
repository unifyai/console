...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data, error, response } = await client.GET('/v0/interfaces/checkpoint', {
      params: {
        query: {
    ...
    if (DEBUG_API && !response.ok) {
      console.warn(
        JSON.stringify({
          route: '/api/interface/checkpoint',
          method: 'GET',
          endpoint: '/v0/interfaces/checkpoint',
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data, error, response } = await client.POST('/v0/interfaces/checkpoint', {
      params: {
        query: {
          interface_id: interfaceId || undefined,
          project_name: projectName || undefined,
