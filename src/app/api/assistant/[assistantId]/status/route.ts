...
export async function GET(
    ...
    try {
        ...
        if (!response.ok) {
             console.error(`Backend Error (assistant status - ${response.status}) for assistant ${params.assistantId}:`, responseData);
        ...
        console.error(`Error proxying to backend for assistant status (assistant ${params.assistantId}):`, error);
        return NextResponse.json({ detail: "Failed to connect to assistant status service", errorDetails: error.message }, { status: 503 });
    }
}