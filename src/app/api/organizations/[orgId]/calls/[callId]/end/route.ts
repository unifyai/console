...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    const response = await fetch(
      `${ORCHESTRA_URL}/v0/organizations/${organizationId}/calls/${encodeURIComponent(callId)}/end`,
      {
        method: 'POST',
        headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(data || { detail: 'Failed to end call' }, {
        status: response.status,
      });
    }
    return NextResponse.json(data, { status: response.status });
  } catch {
    return NextResponse.json({ detail: 'Failed to end call' }, { status: 500 });
  }
}
