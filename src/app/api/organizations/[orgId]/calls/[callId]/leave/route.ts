...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    const response = await fetch(
      `${ORCHESTRA_URL}/v0/organizations/${organizationId}/calls/${encodeURIComponent(callId)}/leave`,
      {
        method: 'POST',
        headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(data || { detail: 'Failed to leave call' }, {
        status: response.status,
      });
    }
    return NextResponse.json(data, { status: response.status });
  } catch {
    return NextResponse.json({ detail: 'Failed to leave call' }, { status: 500 });
  }
}
